...
A green harness that only talks to `omarchy-shell window` and checks `left.x <= right.x` does not pass the OS. The extra proofs, independent source review, and direct metal pointer/pixel/visual campaign establish the current W0 product-windowing baseline. They do not establish Windows 7 Ultimate product completeness.

Start click-through is no longer the open defect it was: outside-click and orb re-toggle landed in `e3bf9385`, and the full-screen swallow was replaced by a compositor-delivered click in `6a548ba9` + `66805cdc`. `test/acceptance.d/start-dismiss-proof.py` is its proof. Current execution and certification requirements are in `plans/ultimate-product-completion-program-2026-08-26.md`; immutable handoffs remain historical evidence.

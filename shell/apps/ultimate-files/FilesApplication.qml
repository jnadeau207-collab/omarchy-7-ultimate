...
Item {
  ...
  Files.ExplorerDetailsPane {
    ...
    boundary: "File contents are never read. New folder runs through files.provider. Open runs through files.provider entry.open and launches the default handler by path. Rename runs through files.provider entry.rename in the same directory. Copy and Paste run through files.provider entry.copy and also place or read files on this session's clipboard. The cut/move write plane exists but is not shell-authorizable. Delete, Restore, and Empty Recycle Bin run through this session's trash helper. Fabric Trash, Restore, and Empty Bin write planes exist but are not shell-authorizable (CHANGES UNAVAILABLE). The empty Recycle Bin write plane exists but is not shell-authorizable under SHELL. Empty Bin LIVE under SHELL remains unavailable. The permanent delete write plane exists but is not shell-authorizable. Recycle product remains unavailable."
    folderPath: {

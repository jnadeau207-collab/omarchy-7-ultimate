...
var OVERVIEW_ROUTE = "oobe.welcome"
...
var ROUTE_QUERIES = [
  {
    routeId: "oobe.network",
    title: "Network",
    providerId: "network.provider",
    action: "inspect",
    capability: "network.inspect",
    supportsResource: true,
    coverage: "Network inventory is readable from network.inspect. Joining a network from setup remains unavailable."
  },
  {
    routeId: "oobe.display",
    title: "Display and scaling",
    providerId: "display.provider",
    action: "inspect",
    capability: "display.inspect",
    supportsResource: true,
    coverage: "Display inventory is readable from display.inspect. Changing resolution or scale from setup remains unavailable."
  },
  {
    routeId: "oobe.migration",
    title: "Bring your files",
    providerId: "files.provider",
    action: "inspect",
    capability: "files.inspect",
    supportsResource: true,
    coverage: "File locations are readable from files.inspect. Importing from a Windows partition remains unavailable."
  },
  {
    routeId: "oobe.recovery",
    title: "Backup and recovery",
    providerId: "recovery.provider",
    action: "inspect",
    capability: "recovery.inspect",
    supportsResource: true,
    coverage: "Restore-point inventory is readable from recovery.inspect. Creating a restore point from setup remains unavailable."
  }
]
...
function normalizedSelection(query, argumentsValue) {
  ...
  if (resourceId !== "") {
    if (!query || !query.supportsResource) throw new Error("This Setup route has no resource selector.")
    if (!opaqueId(resourceId)) throw new Error("The Setup resource ID is invalid.")
  }
  return resourceId
}
...
function requestParameters(query) {
  if (!query || query.providerId === "" || !stableId(query.providerId) || !stableId(query.action))
    throw new Error("The Setup provider query is invalid.")
...
function queryContractError(entry, query) {
  if (!entry || !entry.manifest || entry.manifest.provider !== query.providerId)
    return "The selected provider identity does not match this Setup route."
  ...
  if (!validActionShape(action) || action.mode !== "read" || action.risk !== "read-only" ||
    ...
    return "The provider inventory action does not match the closed read-only Setup contract."
  return ""
}
...
function structuredError(code, title, explanation, detail, recoveryActions) {
  return {
    code: String(code || "oobe.failed"),
    title: clippedText(title || "Setup read failed", 160),
...
function responseError(detail) {
  return structuredError(
    "oobe.invalid-response",
    "Setup rejected provider state",
    "Fabric returned data outside the closed Setup read contract.",
    detail,
    ["fabric.reconnect"]
  )
}
...
function baseState(routeId, argumentsValue, phase) {
  ...
  if (query) {
    ...
    error = responseError("The requested Setup route is not in the closed route map.")
  }
  return {
...
function failureState(previous, errorValue) {
  ...
  var error = isObject(errorValue) ? errorValue : structuredError(
    "oobe.failed", "Setup read failed", String(errorValue || "Unknown Fabric failure.")
  )
...
function validateReadResult(query, entry, result) {
  ...
  if (result.provider !== query.providerId || result.action !== query.action || result.capability !== query.capability)
    return "The provider result does not match the selected Setup route."
  ...
  if (Array.isArray(result.value.resources) && result.value.resources.length > MAX_SOURCE_RECORDS)
    return "The provider resource inventory exceeds the Setup source bound."
  if (query.providerId === "defaults.provider") {
    ...
    if (isObject(result.value.state)) {
      if (!Array.isArray(result.value.state.applications) || result.value.state.applications.length > MAX_SOURCE_RECORDS ||
        ...
        return "The defaults database inventory exceeds the Setup source bound."
    }
  }
  return ""
}
...
function acceptedReadState(previous, result) {
  ...
  if (invalid !== "") {
    var code = invalid.indexOf("obsolete") >= 0 ? "provider.changed-during-read" : "oobe.invalid-response"
...
Controller.prototype._send = function(type, method, params) {
  ...
  if (id === "") {
    var error = this.synchronousFailure || structuredError(
      "oobe.request-rejected",
      "Setup request was rejected",
      "The constrained Fabric client did not accept the read-only Setup request.",
      method,
      ["fabric.reconnect"]
    )
    this.synchronousFailure = null
...
Controller.prototype.markStale = function(requestId) {
  ...
  this._setState(failureState(this.state, structuredError(
    "rpc.timeout", "Setup state became stale",
...
function stateTitle(state) {
  ...
  if (phase === "denied") return "Setup read was denied"
  if (phase === "interrupted") return "Setup read was interrupted"
  if (phase === "stale") return "Setup state is stale"
  if (phase === "failed") return "Setup read failed"
...
function stateExplanation(state) {
  ...
  if (phase === "catalog-loading") return "Setup is reading the current bounded code-owned provider registry before selecting a domain action."
  if (phase === "loading") return "Setup is issuing the exact read-only inventory action declared for this route."
  if (phase === "overview") {
    var available = 0
    ...
    return available + " of " + state.overviewCards.length + " Setup domains have a matching registered read contract. Missing domains remain visibly unavailable."
  }

...
Rectangle {
  ...
  readonly property string infoHonesty: {
    ...
    if (root.sessionInfo.phase === "succeeded")
      return root.sessionInfo.message
    return "System information uses this session's omarchy-fabric-session-apply system-information-inspect helper. It reads OS, product, hardware, and root storage only. session leftover recorded: Settings System information session-UI leftover only (not product CLOSED / not metal CLOSED / not claim=present). Settings does not invent a system-information.provider durable LIVE writer. Settings does not invent Fabric durable system.info.read LIVE / claim=present. Agent Center bubblewrap system.info.read stays separate. Encryption, firmware flash, Device Manager, and storage mutation stay unavailable. Cloud EXIT 0 is not metal leftover CLOSED. windows-native.38 stays prototype/pending."
  }

...
Rectangle {
  ...
  readonly property string printersHonesty: {
    ...
    if (root.sessionPrinters.phase === "succeeded")
      return root.sessionPrinters.message
    return "Printer inventory, set-default, pause, resume, and test-page use this session's absolute /usr/bin/lpstat /usr/bin/lpoptions /usr/sbin/cupsdisable /usr/sbin/cupsenable /usr/bin/lp helpers through omarchy-fabric-session-apply with tip-true printer.* identities. Credentialed printer URIs are refused. session leftover recorded: Settings Printers inventory and queue session-UI leftover only (not product CLOSED / not metal CLOSED / not claim=present). Soft leftover-attach of catalog printers.manage leftover legacy-direct. Settings does not invent a printers.provider durable writer or Fabric SHELL LIVE queue mutation. Fabric printer.inspect and plan-only queue.plan stay separate. Network Add / driver wizard remains OPEN leftover and is not product-complete. Inventory or set-default alone is not Devices and Printers product-complete. Cloud EXIT 0 is not metal leftover CLOSED. windows-native.32 stays pending. Administration > Printers and scanners remains the bounded printer.inspect host."
  }

...
Rectangle {
  ...
  readonly property string layoutHonesty: {
    ...
    if (root.sessionLayout.phase === "succeeded")
      return root.sessionLayout.message
    return "Keyboard layout uses tip-true SettingsSessionKeyboardLayout.setLayout → input-keyboard-layout (this session's /usr/bin/hyprctl helper through omarchy-fabric-session-apply). Only configured layout identities are accepted. Soft leftover-attaches windows-native.33 to this tip-true Settings > Input plane (not product CLOSED / not metal CLOSED / not claim=present). Soft Ship park: leftover-attach before citing suite EXIT 0 as metal; Cloud mocks do not close windows-native.33. windows-native.33 stays prototype/pending. Never claim=present. Settings does not invent an input.provider keyboard-layout durable writer. Fabric input.inspect stays separate. Pointer, repeat rate, IME, and full locale/region remain unavailable. Layout alone is not locale complete. Cloud EXIT 0 is not metal leftover CLOSED. session leftover recorded: Change keyboard layout leftover-attach only (not product CLOSED / not metal CLOSED / not claim=present)."
  }

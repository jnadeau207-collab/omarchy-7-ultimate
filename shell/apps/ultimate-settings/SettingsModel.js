...
function declaredOpsHonesty(routeId) {
  if (String(routeId || "") === "settings.display.overview")
    return "Brightness applies through preflight, approval, and the durable coordinator. Night light uses tip-true SettingsSessionNightlight.setEnabled → NightlightService (same plane as Quick Settings on this session). Scale uses this session's omarchy-hyprland-monitor-scaling helper. Fabric display.inspect stays separate. Settings does not invent a display.provider night-light durable writer. Settings does not invent a display.provider scale durable writer. soft leftover-attaches windows-native.34 to this tip-true Settings > Display night-light plane (not product CLOSED / not metal CLOSED / not claim=present). Soft Ship park: leftover-attach before citing suite EXIT 0 as metal; Cloud mocks do not close windows-native.34. windows-native.34 stays prototype/pending. Never claim=present."

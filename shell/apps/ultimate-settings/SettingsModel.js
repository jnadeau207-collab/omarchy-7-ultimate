...
var ROUTE_QUERIES = [
  {
    ...
    coverage: "Display inventory is readable from display.inspect (connector, mode, scale, position), and brightness applies through display.provider brightness.set on outputs that expose a controllable backlight. Night light uses tip-true SettingsSessionNightlight.setEnabled → NightlightService (same session/heritage plane as Superbar > Quick Settings); Settings hosts that control here and does not invent a display.provider night-light durable writer. Scale applies through tip-true SettingsSessionScaling.setScale → display-monitor-scale (omarchy-hyprland-monitor-scaling whitelist); Settings does not invent a display.provider scale durable writer. Fabric display.inspect stays separate. Resolution, arrangement, and HDR remain unavailable from Settings. Night light or scaling alone is not modern display complete. session leftover recorded: Enable night light soft leftover-attaches windows-native.34 to this tip-true Settings > Display night-light plane (not product CLOSED / not metal CLOSED / not claim=present). Soft Ship park: leftover-attach before citing suite EXIT 0 as metal; Cloud mocks do not close windows-native.34. windows-native.34 stays prototype/pending. Never claim=present. session leftover recorded: Set display scaling soft leftover-attaches windows-native.3 to this tip-true Settings > Display scale plane (not product CLOSED / not metal CLOSED / not claim=present). Soft Ship park: leftover-attach before citing suite EXIT 0 as metal; Cloud mocks do not close windows-native.3. windows-native.3 stays prototype/pending. Never claim=present."
  },
  {
  ...
  },
  {
    ...
    supportsResource: true,
    coverage: "Printer inventory is readable through tip-true SettingsSessionPrinters.readStatus → omarchy-fabric-session-apply printer-status helper (/usr/bin/lpstat -v/-d/-a, tip-true printer.* identities; credentialed URIs refused). Set-default, pause, resume, and test-page apply through tip-true SettingsSessionPrinters.setDefault / pauseQueue / resumeQueue / testPage → printer-default-set / printer-pause / printer-resume / printer-test-page FixedArgv helpers. Soft leftover-attach of catalog printers.manage leftover legacy-direct claim=partial visible Settings > Printers. session leftover recorded: soft leftover-attach ACC windows-native.32 via tip-true SettingsSessionPrinters.setDefault → printer-default-set (multi-verb session surface); Settings Printers leftover-attach only (not product CLOSED / not metal CLOSED / not claim=present). Settings does not invent a printers.provider durable writer or Fabric SHELL LIVE queue mutation. Fabric printer.inspect and plan-only queue.plan stay separate. Network Add / driver wizard remains OPEN leftover. Inventory or set-default alone is not Devices and Printers product-complete. windows-native.32 stays prototype/pending. Cloud mocks do not close windows-native.32. Administration > Printers and scanners remains the bounded printer.inspect host."
  },
  {
  ...
  },
  {
    ...
    supportsResource: false,
    coverage: "OS, product, hardware, and root storage are readable through tip-true SettingsSessionSystemInformation.readInformation → apply_system_information_inspect (this session's omarchy-fabric-session-apply helper). Settings does not invent a system-information.provider durable writer. Settings does not invent Fabric durable system.info.read LIVE / claim=present. Agent Center bubblewrap system.info.read stays separate. Encryption, firmware flash, Device Manager, and storage mutation remain unavailable. session leftover recorded: Find system/storage information soft leftover-attaches windows-native.38 to this tip-true Settings > System information plane (not product CLOSED / not metal CLOSED / not claim=present). Soft Ship park: leftover-attach before citing suite EXIT 0 as metal; Cloud mocks do not close windows-native.38. windows-native.38 stays prototype/pending. Never claim=present."
  }]
...
function declaredOpsHonesty(routeId) {
  if (String(routeId || "") === "settings.display.overview")
    return "Brightness applies through preflight, approval, and the durable coordinator. Night light uses tip-true SettingsSessionNightlight.setEnabled → NightlightService (same plane as Quick Settings on this session). Scale uses tip-true SettingsSessionScaling.setScale → display-monitor-scale (omarchy-hyprland-monitor-scaling whitelist). Fabric display.inspect stays separate. Settings does not invent a display.provider night-light durable writer. Settings does not invent a display.provider scale durable writer. soft leftover-attaches windows-native.34 to this tip-true Settings > Display night-light plane (not product CLOSED / not metal CLOSED / not claim=present). Soft Ship park: leftover-attach before citing suite EXIT 0 as metal; Cloud mocks do not close windows-native.34. windows-native.34 stays prototype/pending. Never claim=present. soft leftover-attaches windows-native.3 to this tip-true Settings > Display scale plane (not product CLOSED / not metal CLOSED / not claim=present). Soft Ship park: leftover-attach before citing suite EXIT 0 as metal; Cloud mocks do not close windows-native.3. windows-native.3 stays prototype/pending. Never claim=present."
  ...
  if (String(routeId || "") === "settings.printers.overview")
    return "Printer inventory, set-default, pause, resume, and test-page use tip-true SettingsSessionPrinters.setDefault / pauseQueue / resumeQueue / testPage / readStatus → omarchy-fabric-session-apply printer-status / printer-default-set / printer-pause / printer-resume / printer-test-page helpers. Soft leftover-attach ACC windows-native.32; windows-native.32 stays prototype/pending. Fabric printer.inspect stays separate. Settings does not invent a printers.provider durable writer. Network Add remains OPEN leftover."
  ...
  if (String(routeId || "") === "settings.system.overview")
    return "System information uses tip-true SettingsSessionSystemInformation.readInformation → apply_system_information_inspect (this session's omarchy-fabric-session-apply helper). Settings exposes no preflight, approval, or execution control for this domain. Settings does not invent a system-information.provider durable writer. Agent Center bubblewrap system.info.read stays separate. soft leftover-attaches windows-native.38 to this tip-true Settings > System information plane (not product CLOSED / not metal CLOSED / not claim=present). Soft Ship park: leftover-attach before citing suite EXIT 0 as metal; Cloud mocks do not close windows-native.38. windows-native.38 stays prototype/pending. Never claim=present."

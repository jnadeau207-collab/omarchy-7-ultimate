...
| Search | prototype | Start `SearchBox` filters installed apps plus existing Settings pages and Start places (Display, Sound, Network, Bluetooth, Power, Personalization, Apps, Update, Recovery, Input, Files, Pictures, Computer/This PC, Agent Center, Settings). Those hits reuse the same desktop Actions / `omarchy-launch-*` paths as the jump list and right-pane places. Not file-content search. Accessibility is not invented as a toggle destination. |

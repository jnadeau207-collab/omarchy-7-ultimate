...
|-----|--------|------------|
| Desktop (icons, wallpaper, context menu, Recycle) | prototype | Wallpaper picker (`omarchy.image-picker`) exists. Desktop Mode shows real XDG Desktop icons (`omarchy.desktop-icons`, `desktopIcons` true). Recycle and a full desktop context menu are still Phase 6. |
...
| Start | prototype | `omarchy.ultimate-start` is a two-pane 720×640 Start: search, pinned, All programs on the left; account, Files, Pictures, Computer, Settings, Agent Center, and power on the right. Idle list hides Terminal/Vim. Outside-click and orb re-toggle: `e3bf9385`. Click-through (compositor delivers the outside click; no full-screen swallow): `6a548ba9` + `66805cdc`, proved by `start-dismiss-proof.py`. |

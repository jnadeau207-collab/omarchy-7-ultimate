...
| Explorer / This PC | prototype | `org.omarchy.Files` is a snappable product window with Home, This PC, Desktop, Documents, Downloads, Pictures, Recent, Search, Trash, and Network routes over `files.provider`. Start Computer uses the published This PC desktop action; Start Pictures uses the Pictures action. This is not Dolphin and not a full Explorer; Nautilus remains the `SUPER + E` path. |

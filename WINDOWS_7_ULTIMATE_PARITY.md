...
| Start | prototype | `omarchy.ultimate-start` is a two-pane 720×640 Start: search, pinned, Recent from real launches, and letter-grouped All programs on the left; account plus iconed Files, Pictures, Computer, Settings, Agent Center, and power on the right. Idle list hides Terminal/Vim. Right-click reuses Superbar jump lists and pin/unpin; Files exposes This PC / Desktop / Documents / Downloads / Pictures / Recent / Trash / Search once the product launcher is published. In-app Files Search stays off Start search. Pictures uses that action instead of `xdg-open`. The power flyout (Lock / Restart / Log off / Shut down) is an in-card menu so click-through cannot steal the caret. Outside-click and orb re-toggle: `e3bf9385`. Click-through (compositor delivers the outside click; no full-screen swallow): `6a548ba9` + `66805cdc`, proved by `start-dismiss-proof.py`. |
| Search | prototype | Start `SearchBox` filters installed apps plus existing Settings pages, Agent Center sections, and Start places (Display, Sound, Network, Bluetooth, Power, Personalization, Apps, Input, Update, Recovery, Files, Pictures, Computer/This PC, Desktop, Documents, Downloads, Recent, Agent Center Tasks/Approvals/Automations/Activity/History/Context/Usage/Permissions/Providers/Artifacts/Troubleshooting, Settings). Those hits reuse the published Settings/Files/Agent Center desktop Actions. Not file-content search. Accessibility and System information are not invented as toggle destinations. Agent Center hits open the existing inspect pages; they do not invent consent or provider writers. The Settings jump list publishes Settings home; Start search still opens Settings itself. Home, Network, Trash, Music, Videos, and in-app Files Search stay off Start search (default Home launch; Sharing/Recycle/MIME are not invented). |

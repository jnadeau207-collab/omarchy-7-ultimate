...
| Search | prototype | Start `SearchBox` filters installed apps plus existing Settings pages, Agent Center sections, and Start places (Display, Sound, Network, Bluetooth, Power, Personalization, Apps, Input, Update, Recovery, Files, Pictures, Computer/This PC, Agent Center Tasks/Approvals/Automations/Activity/History/Context/Usage, Settings). Those hits reuse the published Settings/Files/Agent Center desktop Actions. Not file-content search. Accessibility is not invented as a toggle destination. Consent and provider operations stay outside Agent Center search. |
...
| Agent Center | prototype | `org.omarchy.AgentCenter` is a snappable Superbar pin and Start destination. The pin jump list publishes Tasks, Approvals, Automations, Activity, History, Context, and Usage. Overview reads owner-scoped managed-work counts from Fabric. Inspect tasks can be created, run, cancelled, and recovered for `system.info.read`. Context capture covers the five desktop sources. Consent and provider operations stay outside Agent Center. Usage stays one section. `omarchy.agents` remains a launch shim. |

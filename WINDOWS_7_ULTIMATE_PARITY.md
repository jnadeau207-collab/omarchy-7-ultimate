...
| Agent Center | prototype | `org.omarchy.AgentCenter` is a snappable Superbar pin and Start destination. The pin jump list publishes every existing inspect page: Tasks, Approvals, Automations, Activity, History, Context, Usage, Permissions, Providers, Artifacts, and Troubleshooting. Overview reads owner-scoped managed-work counts from Fabric. Inspect tasks can be created, run, cancelled, and recovered for `system.info.read`. Context capture covers the five desktop sources. Consent and provider operations stay outside Agent Center. Usage stays one section. `omarchy.agents` remains a launch shim. |

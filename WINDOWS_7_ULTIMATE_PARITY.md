...
| Search | prototype | Start `SearchBox` filters installed apps plus existing Settings pages, Agent Center sections, and Start places (Display, Sound, Network, Bluetooth, Power, Personalization, Apps, Input, Update, Recovery, Files, Pictures, Computer/This PC, Desktop, Documents, Downloads, Agent Center Tasks/Approvals/Automations/Activity/History/Context/Usage/Permissions/Providers/Artifacts/Troubleshooting, Settings). Those hits reuse the published Settings/Files/Agent Center desktop Actions. Not file-content search. Accessibility is not invented as a toggle destination. Agent Center hits open the existing inspect pages; they do not invent consent or provider writers. Home, Network, and in-app Files Search stay off the jump list (default Home launch; Sharing is not invented). |

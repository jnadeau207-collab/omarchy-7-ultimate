...
[[ $valid_output == *"27 IPC endpoints/"* ]] || fail "product-contract checker reports every live shell and standalone-app IPC target" "$valid_output"
[[ $valid_output == *"26 applications (24 first-party launchers, 3 shipped pins, 0 planned absent)"* ]] || fail "product-contract checker reports launcher, pin, and standalone-app identities" "$valid_output"

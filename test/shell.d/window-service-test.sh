...
assertEqual(shipped[1].desktopId, 'org.omarchy.Files', 'shipped Files pin is the product Files host')
assertEqual(shipped[2].name, 'Agent', 'shipped pins include Agent')
assertEqual(shipped[2].desktopId, 'org.omarchy.Agent', 'shipped Agent pin launches the Omarchy coding agent')
...
const chromeBox = m.frameBox(wantRect, { class: 'google-chrome' })
assertEqual(chromeBox.x, 288, 'Chromium box starts an inset left of the wanted rect')

...
payload = entry_payload("files.location.documents", "report.txt", source, "files.location.documents", "", "memo.txt")
status, result = run(payload)
moved = home / "Documents" / "memo.txt"
check("a same-directory move writes the destination", status == 0 and result.get("ok") and result.get("moved") and moved.is_file(), json.dumps(result))
check("the source name is gone after a same-directory move", not source.exists(), str(source.exists()))
...
check("rollback desired restores the source name", status == 0 and result.get("ok") and source.is_file() and not moved.exists(), json.dumps(result))
...
status, result = run(payload)
desktop_moved = home / "Desktop" / "report.txt"
check("a cross-location move writes into the destination location", status == 0 and result.get("ok") and desktop_moved.is_file(), json.dumps(result))
check("cross-location move removes the source", not source.exists(), str(source.exists()))
check("cross-location destination holds the source bytes", desktop_moved.read_text(encoding="utf-8") == "hello" if desktop_moved.exists() else False, desktop_moved.read_text(encoding="utf-8") if desktop_moved.exists() else "missing")
...
missing_parent = dict(payload)
missing_parent["destinationParentRelativePath"] = "missing-folder"
missing_parent["resourceId"] = directory_resource("files.location.documents", "missing-folder", payload["entryId"])
status, result = run(missing_parent)
check("a missing destination parent refuses instead of creating it", result.get("code") == "resource.unresolved", json.dumps(result))
check("dir-as-needed leftover stays closed", not (home / "Documents" / "missing-folder").exists(), str((home / "Documents" / "missing-folder").exists()))
...
folder.mkdir()
status, result = run(entry_payload("files.location.documents", "reports", folder, "files.location.documents", "", "reports-moved"))
...
check("Trash refuse does not move the file", trash_file.is_file() and not (home / "Documents" / "restored.txt").exists(), str(trash_file.exists()))
...
check("Trash destination refuse leaves the source", source.is_file(), str(source.exists()))

if failures:
    print("\n".join(f"not ok - {item}" for item in failures), file=sys.stderr)
    raise SystemExit(1)
PY
pass "the Move helper relocates a scoped file and refuses drift, traversal, collision, missing parents, parent-only scope, symlinks, directories, and Trash"

...
const documentsDest = search.sortedEntries(destApps.concat(search.searchDestinations('documents', destApps)), 'documents')
assertEqual(search.entryName(documentsDest[0].entry), 'Documents', 'documents ranks Files Documents')
assertEqual(documentsDest[0].entry.actionId, 'Documents', 'Documents uses the Files jump action')
const downloadsDest = search.sortedEntries(destApps.concat(search.searchDestinations('downloads', destApps)), 'downloads')
assertEqual(search.entryName(downloadsDest[0].entry), 'Downloads', 'downloads ranks Files Downloads')
assertEqual(downloadsDest[0].entry.actionId, 'Downloads', 'Downloads uses the Files jump action')
const recentDest = search.sortedEntries(destApps.concat(search.searchDestinations('recent', destApps)), 'recent')
assertEqual(search.entryName(recentDest[0].entry), 'Recent', 'recent ranks Files Recent')
assertEqual(recentDest[0].entry.actionId, 'Recent', 'Recent uses the Files jump action')
...
assert(destNames.indexOf('Desktop') >= 0, 'destinations include Files Desktop')
assert(destNames.indexOf('Documents') >= 0, 'destinations include Files Documents')
...
assert(!search.START_DESTINATIONS.some(row => row.actionId === 'Overview'), 'destinations do not invent a second Agent Center Overview')
assert(!search.START_DESTINATIONS.some(row => row.actionId === 'Home'), 'destinations do not invent a second Settings home or Files Home')
...
assert(destNames.indexOf('Pending Approvals') >= 0, 'destinations include Agent Center Approvals')
assert(destNames.indexOf('Automations') >= 0, 'destinations include Agent Center Automations')
assert(destNames.indexOf('Activity & operations') >= 0, 'destinations include Agent Center Activity')
assert(destNames.indexOf('History') >= 0, 'destinations include Agent Center History')
assert(destNames.indexOf('Context') >= 0, 'destinations include Agent Center Context')
assert(destNames.indexOf('Usage') >= 0, 'destinations include Agent Center Usage')
assert(destNames.indexOf('Permissions & trust') >= 0, 'destinations include Agent Center Permissions')
assert(destNames.indexOf('Providers & accounts') >= 0, 'destinations include Agent Center Providers')
...
const historyDest = search.sortedEntries(destApps.concat(search.searchDestinations('history', destApps)), 'history')
assertEqual(search.entryName(historyDest[0].entry), 'History', 'history ranks Agent Center History')
assertEqual(historyDest[0].entry.actionId, 'History', 'History uses the Agent Center jump action')
const contextDest = search.sortedEntries(destApps.concat(search.searchDestinations('context', destApps)), 'context')
assertEqual(search.entryName(contextDest[0].entry), 'Context', 'context ranks Agent Center Context')
assertEqual(contextDest[0].entry.actionId, 'Context', 'Context uses the Agent Center jump action')
const usageDest = search.sortedEntries(destApps.concat(search.searchDestinations('usage', destApps)), 'usage')
assertEqual(search.entryName(usageDest[0].entry), 'Usage', 'usage ranks Agent Center Usage')
assertEqual(usageDest[0].entry.actionId, 'Usage', 'Usage uses the Agent Center jump action')
const permissionsDest = search.sortedEntries(destApps.concat(search.searchDestinations('permissions', destApps)), 'permissions')
assertEqual(search.entryName(permissionsDest[0].entry), 'Permissions & trust', 'permissions ranks Agent Center Permissions')
assertEqual(permissionsDest[0].entry.actionId, 'Permissions', 'Permissions uses the Agent Center jump action')
const artifactsDest = search.sortedEntries(destApps.concat(search.searchDestinations('artifacts', destApps)), 'artifacts')
assertEqual(search.entryName(artifactsDest[0].entry), 'Artifacts', 'artifacts ranks Agent Center Artifacts')
assertEqual(artifactsDest[0].entry.actionId, 'Artifacts', 'Artifacts uses the Agent Center jump action')
const troubleshootingDest = search.sortedEntries(destApps.concat(search.searchDestinations('troubleshooting', destApps)), 'troubleshooting')
assertEqual(search.entryName(troubleshootingDest[0].entry), 'Troubleshooting', 'troubleshooting ranks Agent Center Troubleshooting')
assertEqual(troubleshootingDest[0].entry.actionId, 'Troubleshooting', 'Troubleshooting uses the Agent Center jump action')

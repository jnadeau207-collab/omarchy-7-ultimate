...
const stuck = new FakeSocketHarness({ requestTimeoutMs: 1000, reconnectBaseMs: 100, reconnectMaxMs: 400 })
stuck.start()
assertEqual(stuck.connects, 1, 'start opens one connection')
assert(stuck.engine.connectInFlight, 'start leaves the connect in flight until the socket opens')
stuck.tick(999)
assertEqual(stuck.connects, 1, 'a live connect waits for its deadline')
assertEqual(stuck.closes, 0, 'a live connect does not drop the socket before the deadline')
stuck.tick(1000)
assertEqual(stuck.closes, 1, 'a timed-out connect drops the stuck socket')
assertEqual(stuck.engine.state, 'reconnecting', 'a timed-out connect schedules reconnect')
stuck.tick(1100)
assertEqual(stuck.connects, 2, 'auto-reconnect runs after a stuck connect times out')

const explicit = new FakeSocketHarness()
explicit.start()
assertEqual(explicit.connects, 1, 'explicit retry starts from a connecting client')
assert(explicit.engine.retry(explicit.now), 'retry unsticks a connecting client')
assertEqual(explicit.closes, 1, 'retry drops the stuck socket')
assertEqual(explicit.connects, 2, 'retry opens a new connection immediately')
...
assert(/^Item \{/m.test(qml) && /Socket \{/.test(qml), 'QML adapter keeps its Quickshell Unix Socket behind component scope')
assert(/_recycleWire/.test(qml) && /wireLoader\.active = false/.test(qml) && /Qt\.callLater/.test(qml), 'QML adapter destroys and recreates Socket for a replaced fabric.sock inode')
assert(/SplitParser\s*\{[\s\S]*?splitMarker: ""/.test(qml), 'QML adapter receives arbitrary chunks for bounded newline framing')
assert(/sock\.write\(frame\)[\s\S]*?sock\.flush\(\)/.test(qml), 'QML adapter writes and flushes each complete bounded frame')

...
grep -Fqx '  fabricAllowedMethods: ["managed-work.query", "managed-work.task.create", "managed-work.task.cancel", "managed-work.task.recover", "managed-work.context.capture", "managed-work.run.execute"]' "$ROOT/shell/ultimate-agent-center.qml" || fail "Agent Center has a bounded Fabric method allowlist"

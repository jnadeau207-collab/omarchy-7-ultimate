...
Honesty addendum 2026-09-05 vs Recycle desktop place: Desktop Recycle icon is the shipped `Recycle Bin.desktop` shortcut. Double-click runs `omarchy-launch-files --source desktop files.trash` through the same uwsm argv path as Computer. Files pins `emptyBinAuthorized=false` and does not offer Empty Bin LIVE. Restore UI stays unavailable. Recycle Properties and a full desktop context menu stay leftovers. Recycle Bin / Empty Bin LIVE residual OPEN after PR #63. Recycle Bin is not product-complete. Do not invent Empty Bin LIVE, Restore LIVE under SHELL, `files.trash.manage` claim=present, or Recycle product-complete. Product REJECTED.
...
The Files banner matches: New folder runs through `files.provider`; Copy and Paste run through `entry.copy` and also place or read files on this session's clipboard; the cut/move write plane exists but is not shell-authorizable; Trash write plane exists but is not shell-authorizable (CHANGES UNAVAILABLE); Restore write plane exists but is not shell-authorizable; the permanent delete write plane exists but is not shell-authorizable; the empty Recycle Bin write plane exists but is not shell-authorizable; Restore UI, Empty Bin LIVE, and Recycle product remain unavailable.

...
Honesty addendum 2026-09-03: `files.entry.copy` write plane is reachable (`files-entry-copy` helper). Real preflight binds source identity inside the destination directory listing. Copy writes a scoped regular-file replica into a Files location. Collision, drift, traversal, symlink, directory, and Trash refuse. Risk is `low` (additive replica, same class as `directory.create`, not a cut/move). SHELL grant is allowed. Catalog claim=partial; humanRoute visible `Files > Copy and Paste`. Agent stays unavailable. Files Copy and Paste are LIVE (`copyAuthorized=true`) with in-app staging only. The OS clipboard, folder copy, thumbnails, Open With / MIME UI, Restore LIVE, Empty Bin, and Explorer product-complete stay leftovers. Recycle and MIME residuals stay OPEN. Product REJECTED.

Honesty addendum 2026-09-03: `files.entry.move` write plane is reachable (`files-entry-move` helper). Real preflight binds source identity inside the destination directory listing. Move relocates a scoped regular file into a writable Files location. Collision, drift, traversal, symlink, directory, missing parent, and Trash refuse. Risk is `consequential` (source disappears). SHELL refused at `grant.shell-consequential`. Catalog claim=partial; humanRoute planned empty. Agent stays unavailable. Files Cut/Paste Move stay gated `cutAuthorized=false`. Do not invent LIVE Cut under SHELL, an OS clipboard, or folder copy. Recycle and MIME residuals stay OPEN. Explorer stays prototype. Product REJECTED.
...
| `files-entry-copy` | Files › Copy and Paste | write plane reachable (`files.entry.copy`, risk `low`; SHELL grant OK; catalog claim=partial; humanRoute visible; Files Copy/Paste LIVE for regular files with in-app staging; collision refuse; no OS clipboard or folder copy) |
| `files-entry-move` | none | write plane reachable (`files.entry.move`, risk `consequential`; SHELL refused at `grant.shell-consequential`; catalog claim=partial; humanRoute planned empty; Files Cut/Paste Move gated `cutAuthorized=false`; collision refuse; no OS clipboard, LIVE Cut, or folder copy) |
...
- `entry.copy` — Files › Copy and Paste. Risk `low`. SHELL may hold a standing grant. Regular files only. Destination is a writable Files location. Collision refuses. Staging is in-app only. The scoped destination listing carries the source entry identity so a names-only TOCTOU cannot hide inode drift.

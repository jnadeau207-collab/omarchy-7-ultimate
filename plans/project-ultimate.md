...
|--------|---------|-------------|-----------------------------|
| Sound | `audio.inspect` | `output-volume.set`; session mute/default (`audio-output-mute-set` / `audio-output-default-set`, soft leftover-attach `audio.output.manage`) | ports, troubleshoot; session leftover only (not product CLOSED / not claim=present); `parity.sound` prototype; `windows-native.6` pending |

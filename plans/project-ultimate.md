...
| Files | `files.browse` | `directory.create` (New folder LIVE); `files.entry.open` (Open LIVE, risk low); `files.entry.rename` (Rename LIVE, risk low, same-directory); `files.entry.copy` (Copy/Paste LIVE, risk low, in-app staging, regular files and directories); `files.entry.move` write plane, CHANGES UNAVAILABLE under SHELL (`cutAuthorized=false`); `files.entry.delete` write plane, CHANGES UNAVAILABLE under SHELL (`deleteAuthorized=false`); `files.entry.trash` and `files.trash.restore` write planes, CHANGES UNAVAILABLE under SHELL | Restore UI (do not invent Restore LIVE), empty Recycle Bin, LIVE Delete, LIVE Cut, OS clipboard, desktop Recycle icon, desktop context menu; `files.trash.manage` missing; Recycle Bin is not product-complete |

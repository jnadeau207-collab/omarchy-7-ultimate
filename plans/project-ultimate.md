...
- Settings (`org.omarchy.Settings`) is a snappable window with caption and nav. Personalization hosts the existing image picker inside that chrome. Display, Sound, Network, Bluetooth, Power, Apps, Update, and Recovery read Fabric `display.inspect` / `audio.inspect` / `network.inspect` / `bluetooth.inspect` / `power.inspect` / `defaults.inspect` / `update.inspect` / `recovery.inspect` instead of embedding Process panels. Accessibility and Input stay honest Fabric pages because no Settings panel exists (keyboard layout is a bar widget; accessibility.configure is planned). The Settings jump list keeps Accessibility and System information as those missing pages; Start search does not invent them. System stays a Fabric page because no aggregate system-information provider is registered. Typed domain writers remain Phase 5. The overlay plugin launches this window instead of toggling floating panels.

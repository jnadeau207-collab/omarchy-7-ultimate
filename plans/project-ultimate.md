...
| Display | `display.inspect` | `brightness.set`; tip-true `SettingsSessionNightlight.setEnabled` → `NightlightService` soft leftover-attached to ACC `windows-native.34` prototype/pending (`display.night-light.set` claim=partial / human=partial; never claim=present) | resolution / scale / arrangement / HDR; `parity.display` prototype; `parity.modern-display-scaling-hdr-night-light` prototype; Soft Ship park before suite EXIT 0 as metal; Cloud mocks do not close windows-native.34; `windows-native.3` pending |

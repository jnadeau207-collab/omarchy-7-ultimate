...
- Settings (`org.omarchy.Settings`) is a snappable window with caption and nav. Personalization hosts the existing image picker inside that chrome. Display, Sound, Network, Bluetooth, Power, Apps, Update, and Recovery read Fabric `display.inspect` / `audio.inspect` / `network.inspect` / `bluetooth.inspect` / `power.inspect` / `defaults.inspect` / `update.inspect` / `recovery.inspect` instead of embedding Process panels. Accessibility and Input stay honest Fabric pages because no Settings panel exists (keyboard layout is a bar widget; accessibility.configure is planned). The Settings jump list publishes Settings home plus the inspect pages and keeps Accessibility and System information as those missing pages; Start search still opens Settings itself rather than inventing a second home destination. Start search does not invent Accessibility or System information. System stays a Fabric page because no aggregate system-information provider is registered. Typed domain writers remain Phase 5. The overlay plugin launches this window instead of toggling floating panels.
...
- Settings window hosts the existing Personalization picker. Display, Sound, Network, Bluetooth, Power, Apps, Update, and Recovery read Fabric `display.inspect` / `audio.inspect` / `network.inspect` / `bluetooth.inspect` / `power.inspect` / `defaults.inspect` / `update.inspect` / `recovery.inspect`. Typed writers remain Phase 5.
...
- Links: `plans/WIN7_SPEC_INDEX.md`, `plans/win7-ultimate-ground-truth/07-interaction-grammar.md` (global metrics), `plans/win7-ultimate-ground-truth/01-window-chrome.md`
...
- Links: `plans/win7-ultimate-ground-truth/01-window-chrome.md`, `plans/win7-ultimate-ground-truth/03-superbar-taskbar.md` (Show Desktop/peeks), `plans/win7-ultimate-ground-truth/07-interaction-grammar.md` (Aero + hotkeys)
...
- Links: `plans/win7-ultimate-ground-truth/07-interaction-grammar.md`, `plans/win7-ultimate-ground-truth/02-start-menu.md` (Agent Center as Start place = 2026 delta)
...
- Hit targets / focus rings / motion: `plans/win7-ultimate-ground-truth/07-interaction-grammar.md` (hover 400 ms, drag 4 px, dialog 75×23 @ 96).
...
- Links: `plans/win7-ultimate-ground-truth/01-window-chrome.md`, `plans/win7-ultimate-ground-truth/07-interaction-grammar.md`, `plans/win7-ultimate-ground-truth/03-superbar-taskbar.md` (40 vs 48 height delta)
...
- **Start:** two-pane; BINDING right-pane order User→Documents→Pictures→Music→Games→Computer→Control Panel→Devices and Printers→Default Programs→Help; power flyout Switch user/Log off/Lock/Restart/Sleep/Hibernate; All Programs **in-pane** with Back; search placeholder “Search programs and files”; orb 54×54 states (`plans/win7-ultimate-ground-truth/02-start-menu.md`). Omarchy 720×640 / Settings/Agent Center places = documented deltas.
- **Superbar:** height 40 (Win7) vs product 48; combine modes; Jump List structure; left-click minimize-active; Shift/middle new instance; tray Action Center/Volume/Network/Power/Clock; Show Desktop (`plans/win7-ultimate-ground-truth/03-superbar-taskbar.md`).
- Desktop icons + context menu order View/Sort/Refresh/Paste/New/Screen resolution/Gadgets/Personalize (`plans/win7-ultimate-ground-truth/04-explorer-dialogs.md`).
...
- Links: `plans/win7-ultimate-ground-truth/02-start-menu.md`, `plans/win7-ultimate-ground-truth/03-superbar-taskbar.md`, `plans/win7-ultimate-ground-truth/04-explorer-dialogs.md` (desktop), `plans/win7-ultimate-ground-truth/07-interaction-grammar.md`
...
- Map Settings pages to Win7 CP applets in `plans/win7-ultimate-ground-truth/05-control-panel.md` (55 items) + `plans/win7-ultimate-ground-truth/06-settings-defaults-admin-media.md` Personalization / Sound / Display / Power / Default Programs.
...
- Links: `plans/win7-ultimate-ground-truth/05-control-panel.md`, `plans/win7-ultimate-ground-truth/06-settings-defaults-admin-media.md`, `plans/win7-ultimate-ground-truth/07-interaction-grammar.md`
...
- Explorer chrome: command bar (not ribbon); nav Favorites→Libraries→Homegroup→Computer→Network; Libraries defaults; views Extra Large…Content; Computer groups + capacity bars; Recycle Bin behaviors (`plans/win7-ultimate-ground-truth/04-explorer-dialogs.md`).
...
- AutoPlay from Default Programs child (`plans/win7-ultimate-ground-truth/06-settings-defaults-admin-media.md`).
- Links: `plans/win7-ultimate-ground-truth/04-explorer-dialogs.md`, `plans/win7-ultimate-ground-truth/06-settings-defaults-admin-media.md`, `plans/win7-ultimate-ground-truth/05-control-panel.md` (Folder Options)
...
- Media player product may reference WMP 12 chrome jobs (`plans/win7-ultimate-ground-truth/06-settings-defaults-admin-media.md`) without clone mandate.
- Links: `plans/win7-ultimate-ground-truth/05-control-panel.md` (Programs and Features), `plans/win7-ultimate-ground-truth/06-settings-defaults-admin-media.md`
...
- Default Programs / AutoPlay remain human path for “which program opens this” (`plans/win7-ultimate-ground-truth/06-settings-defaults-admin-media.md`).
- Links: `plans/win7-ultimate-ground-truth/06-settings-defaults-admin-media.md`, `plans/win7-ultimate-ground-truth/05-control-panel.md`
...
- Each noun → job id: Task Manager (`parity.task-manager`), Device Manager, Disk Management, Services, Task Scheduler, Event Viewer (≠ toast history), Firewall, User Accounts, Backup, Recovery, Troubleshooting (`plans/win7-ultimate-ground-truth/05-control-panel.md`, `plans/win7-ultimate-ground-truth/06-settings-defaults-admin-media.md` MMC trees).
...
- Links: `plans/win7-ultimate-ground-truth/06-settings-defaults-admin-media.md`, `plans/win7-ultimate-ground-truth/05-control-panel.md`, `fleet-doctrine-gaps` Administration rows
...
- Dialog/progress grammar from `plans/win7-ultimate-ground-truth/07-interaction-grammar.md`; no-bullshit install Rule.
- Links: `plans/win7-ultimate-ground-truth/07-interaction-grammar.md`, `plans/win7-ultimate-ground-truth/02-start-menu.md` (power), `plans/win7-ultimate-ground-truth/05-control-panel.md` (Getting Started analog only)
...
- Apply full `plans/win7-ultimate-ground-truth/07-interaction-grammar.md`: hover/drag/menus/dialogs/progress/balloons/Aero gestures/DPI/themes/sounds/cursors/Win+ hotkeys.
...
- Links: `plans/win7-ultimate-ground-truth/07-interaction-grammar.md`, all `01`–`06` ground-truth files, `plans/WIN7_SPEC_INDEX.md`

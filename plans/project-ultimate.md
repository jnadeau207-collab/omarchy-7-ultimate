...
- Settings (`org.omarchy.Settings`) is a snappable window with caption and nav. Personalization hosts the existing image picker inside that chrome. Display, Sound, Network, Bluetooth, and Power read Fabric `display.inspect` / `audio.inspect` / `network.inspect` / `bluetooth.inspect` / `power.inspect` instead of embedding the Process monitor, audio, nmcli, bluetoothctl, and power panels. Accessibility and Input stay honest Fabric pages because no Settings panel exists (keyboard layout is a bar widget; accessibility.configure is planned). Update/Recovery/System stay Fabric pages. Typed domain writers remain Phase 5. The overlay plugin launches this window instead of toggling floating panels.
...
- Settings window hosts the existing Personalization picker. Display, Sound, Network, Bluetooth, and Power read Fabric `display.inspect` / `audio.inspect` / `network.inspect` / `bluetooth.inspect` / `power.inspect`. Typed writers remain Phase 5.
...
Catalog jobs stay honest: most rows remain `claim: "missing"` and `parity.agent-center` is not present. Persistent inspect tasks, five-source context capture (`open-windows`, `focused-application`, `selection`, `virtual-desktops`, `mode-profile`), and bubblewrap `system.info.read` execution are live on the checkout Fabric unit. Settings Display, Sound, Network, Bluetooth, and Power read Fabric `display.inspect` / `audio.inspect` / `network.inspect` / `bluetooth.inspect` / `power.inspect`. Superbar still hosts Sound/Network/Bluetooth/Power panels. Typed writers remain Phase 5.

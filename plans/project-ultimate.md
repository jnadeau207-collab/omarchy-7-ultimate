...
| Apps | `defaults.inspect` | `protocol.set` (default browser) | other associations / startup / Default Programs applet; MIME / Default Programs association UI residual OPEN after PR #62; `defaults.mime.set` write plane reachable, Settings does not offer MIME LIVE CONTROL; `files.associations.set` stays missing/planned MIME; `parity.default-programs` prototype; `windows-native.19` pending |

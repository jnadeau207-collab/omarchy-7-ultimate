...
- Jump lists and Agent Center UI are on the Superbar/Start path. Peek captures live window thumbnails. Live Settings writers are Sound volume, Network Wi-Fi radio, Display brightness, and Apps default browser; Input layout uses this session leftover plane; Bluetooth pair and connect use this session's BlueZ adapter; Power stays inspect-only because polkit cannot authorize profile.set from app.slice; other domains stay inspect-only or host the existing Personalization picker. The Settings window already hosts those existing panels.
...
- Desktop Computer is the shipped `Computer.desktop` shortcut. Double-click runs the same `files.this-pc` launch as Start Computer, through `AppLibrary.launchCommand` and `uwsm-app`, not `xdg-open` of the desktop file. Desktop Recycle Bin is the shipped `Recycle Bin.desktop` shortcut. Double-click runs the same `files.trash` launch as Start Recycle Bin, through the same uwsm argv path. Empty Bin LIVE stays unauthorized (`emptyBinAuthorized=false`).
- Superbar is still job-incomplete as a finished Windows taskbar (no Recycle, no Task Manager, no full context-menu API). Catalog `process.inspect` is visible on Administration > Processes with CPU share (FixedArgv `/usr/bin/ps` `pcpu`; claim=partial / human=partial soft leftover-attached); honest-unavailable as Task Manager product — do not invent Superbar > Task Manager; `windows-native.26` stays prototype/pending; End Task stays OPEN leftover (`terminationAuthorized=false`). Leftover `processes.inspect` (`processes.provider` / `legacy-direct` / `claim=partial` / Task Manager surface / btop source) is closed. Desktop icon manage and desktop context-menu stay unpublished empty routes. Administration inspect readers (accounts, devices, firewall, printers, services, schedules, storage, plus Processes, Backup, and Troubleshooting) are visible on live Administration titles after #36; other Admin inspect claims stay missing while `process.inspect` is soft leftover-attached claim=partial for the CPU plane; Phase 9 exit criteria still open; inspect ≠ MMC product present. End Task stays unauthorized (`terminationAuthorized=false`); do not invent present Settings, Start, or Superbar Administration pages. Presentation now includes the Start orb, stacked-window Task View mark, drawn Quick Settings and Notification Center marks, two-line clock, live peeks, jump lists, structured previews, a stacked tile edge for multi-window groups, and hover tooltips. Task buttons carry no count badges. Multi-monitor policy is landed: the primary output (Hyprland id 0, else leftmost) owns the notification cluster; every bar shows pins; a secondary bar also shows running groups that have a window on that output; Start and Task View open on the Superbar that summoned them. Jump list Close window closes the active window; Close group still closes every window in the group. Peek × closes one. Notification-area cluster is driven from `barConfig.layout.right` + `BarWidgetRegistry` (`TrayCluster.qml`). Desktop Mode overlay includes Quick Settings, Notification Center, agents, the existing `omarchy.system-update` and `omarchy.keyboard-layout` widgets, tray, and clock without rewriting `shell.json`. Chrome hexes include `#1b1b1b` `#333333` `#3a3a3a` `#4a4a4a` `#e8943a` `#9cbc0d` `#55ffffff`. Caption min/max/close colors are `Tokens.caption` via the generated chrome adapter — not a second private file and not a hard-coded `rgba(1a1a1acc)`. `Variants` still draws a bar per `Quickshell.screens`.
- Start is a two-pane 720×640 launcher (search + pins + letter-grouped All programs on the left; account, iconed Files/Pictures/Computer/Settings/Agent Center, and power on the right). Search ranks installed apps plus existing Settings pages, Agent Center sections, and Start places; Display/Sound/Network/Bluetooth/Power/Personalization/Apps/Input/Update/Recovery/Pictures/Computer/Desktop/Documents/Downloads/Recent and Agent Center Tasks/Approvals/Automations/Activity/History/Context/Usage/Permissions/Providers/Artifacts/Troubleshooting reuse the published jump actions. The Agent Center jump list also publishes Overview; Start search still opens Agent Center itself rather than inventing a second Overview destination. Idle All programs stays apps-only. Not file-content search. Recent is the last launched programs that are not already pinned, persisted in `start-recents.json` from `AppLibrary.launch`. Right-click on pins, Recent, All programs, and Start places with a desktop id opens the same Superbar verbs: desktop-entry jump list plus Pin to taskbar / Unpin from taskbar. Files jump lists include Home, This PC, Desktop, Documents, Downloads, Pictures, Recent, Trash, and Search after the product launcher is published. Start search still opens Files itself rather than inventing a second Home destination. Files Home stays degraded. Computer uses that This PC action; Pictures uses the Pictures action. Both open product Files, not Nautilus. The power flyout (Lock / Restart / Log off / Shut down) lives on the Start card like the pin menu, so click-through cannot steal the caret click. Shut down soft leftover-attaches ACC `windows-native.40` via tip-true `shutdown` → `omarchy-system-shutdown` (`power.shutdown` claim=partial / human=partial visible Start > Shut down; never claim=present; not product CLOSED / not metal CLOSED; `windows-native.40` stays prototype/pending; Cloud mocks do not close windows-native.40). Outside-click, orb re-toggle, the shared transient coordinator, and compositor-delivered click-through are landed and proved. Start owns the output named by the clicked orb (`PanelWindow.screen` + `start-owner.json` for the click-through hit test). Closing the same orb restores the previously focused window; wallpaper/window click-through does not.
- Settings (`org.omarchy.Settings`) is a snappable window with caption and nav. Personalization hosts the existing image picker inside that chrome. Display, Sound, Network, Bluetooth, Power, Apps, Update, and Recovery read Fabric `display.inspect` / `audio.inspect` / `network.inspect` / `bluetooth.inspect` / `power.inspect` / `defaults.inspect` / `update.inspect` / `recovery.inspect` instead of embedding Process panels. Input layout soft leftover-attaches ACC `windows-native.33` via tip-true `SettingsSessionKeyboardLayout.setLayout` → `input-keyboard-layout` (`input.keyboard-layout.set` claim=partial; never claim=present; `windows-native.33` stays prototype/pending; Cloud mocks do not close windows-native.33); Settings does not invent an `input.provider` keyboard-layout durable writer; Accessibility stays an honest missing Fabric page because no `accessibility.provider` is registered. The Settings jump list publishes Settings home plus the inspect pages, keeps Accessibility as an honest missing page, and publishes System information for the session read leftover; Start search still opens Settings itself rather than inventing a second home destination. Start search does not invent Accessibility. System information soft leftover-attaches ACC `windows-native.38` via tip-true `SettingsSessionSystemInformation.readInformation` → `apply_system_information_inspect` (`system.info.read` claim=partial; never claim=present; `windows-native.38` stays prototype/pending; Cloud mocks do not close windows-native.38); Settings does not invent a Fabric `system-information.provider` durable writer or claim=present. Printers soft leftover-attaches ACC `windows-native.32` via tip-true `SettingsSessionPrinters.setDefault` → `printer-default-set` (`printers.manage` claim=partial; never claim=present; `windows-native.32` stays prototype/pending; Cloud mocks do not close windows-native.32; Network Add / driver wizard remains OPEN leftover); Settings does not invent a `printers.provider` durable writer. Live Settings writers are listed in Current position; remaining domain verbs stay unavailable and jobs stay prototype/pending. The overlay plugin launches this window instead of toggling floating panels.
...
|--------|---------|-------------|-----------------------------|
| Sound | `audio.inspect` | `output-volume.set`; session mute/default (`audio-output-mute-set` / `audio-output-default-set`, soft leftover-attach `audio.output.manage`); session audio restart (`audio-troubleshoot-restart`, soft leftover-attach `troubleshooting.audio.run`) | ports, full troubleshoot wizard OPEN leftover; session leftover only (not product CLOSED / not claim=present); `parity.sound` prototype; `windows-native.6` pending; `windows-native.39` prototype/pending |
| Network | `network.inspect` | `wifi.set-enabled`; session join (open/PSK) | enterprise / VPN / per-connection DNS; `parity.network` prototype; `windows-native.2` pending |
...
| Display | `display.inspect` | `brightness.set`; tip-true `SettingsSessionNightlight.setEnabled` → `NightlightService` soft leftover-attached to ACC `windows-native.34` prototype/pending (`display.night-light.set` claim=partial / human=partial; never claim=present); tip-true `SettingsSessionScaling.setScale` → `display-monitor-scale` / `omarchy-hyprland-monitor-scaling` soft leftover-attached to ACC `windows-native.3` prototype/pending (`display.scale.set` claim=partial / human=partial; never claim=present) | resolution / arrangement / HDR; `parity.display` prototype; `parity.modern-display-scaling-hdr-night-light` prototype; Soft Ship park before suite EXIT 0 as metal; Cloud mocks do not close windows-native.34; Cloud mocks do not close windows-native.3 |
| Input / Language | `input.inspect` | tip-true `SettingsSessionKeyboardLayout.setLayout` → `input-keyboard-layout` soft leftover-attached to ACC `windows-native.33` prototype/pending (`input.keyboard-layout.set` claim=partial / human=partial; never claim=present) | pointer / repeat / locale; `parity.language-locale` prototype; Soft Ship park before suite EXIT 0 as metal; Cloud mocks do not close windows-native.33 |
| Apps | `defaults.inspect` | `protocol.set` (default browser http+https scheme-by-scheme, default email mailto on Settings > Default Programs), `mime.set` (writable multi-candidate associations) | MIME / Default Programs association UI residual OPEN after PR #62; Settings > Default Programs offers MIME LIVE CONTROL for writable multi-candidate associations; Settings > Apps hosts tip-true XDG autostart enable/disable via `SettingsSessionStartup.setEnabled` → `apps-startup-set` soft leftover-attached to ACC `windows-native.27` prototype/pending (not product CLOSED / not metal CLOSED / not claim=present; Cloud mocks do not close windows-native.27); other associations / Default Programs applet; `files.associations.set` stays missing/planned MIME; `parity.default-programs` prototype; session leftover recorded: soft leftover-attach ACC `windows-native.19` to tip-true Settings > Default Programs browser plane via `defaults.protocol.set` (not product CLOSED / not metal CLOSED / not claim=present); `windows-native.19` stays prototype/pending; Cloud mocks do not close windows-native.19; do not invent AutoPlay/SPAD/`files.associations.set` product-complete |
| Bluetooth | `bluetooth.inspect` | session pair/connect (BlueZ) | audio routing / PIN-entry / rfkill; `windows-native.5` pending |
| Printers | `printer.inspect` (Administration) | tip-true `SettingsSessionPrinters.setDefault` → `printer-default-set` (multi-verb pause/resume/test-page/status) soft leftover-attached to ACC `windows-native.32` prototype/pending (`printers.manage` claim=partial / human=partial; never claim=present) | Network Add / driver wizard OPEN leftover; Devices and Printers product; Soft Ship park before suite EXIT 0 as metal; Cloud mocks do not close windows-native.32 |
| Update | `update.inspect` | session apply (`omarchy-update`); session history (`system-update-history` / `readHistory`); Fabric `system.update` not LIVE | restart / reboot; `parity.update` prototype; `update.history.read` leftover partial soft leftover-attached to `windows-native.29` prototype/pending; `windows-native.28` pending until human/metal proof |
| Recovery | `recovery.inspect` | none | restore plan / exec; `windows-native.30–31` pending |
| Personalization | hosted Process picker (`ImagePicker.applyEmbedded` → `omarchy-theme-bg-set`) | wallpaper soft leftover-attached via `desktop.wallpaper.set` claim=partial visible Settings > Personalization; soft leftover-attach ACC `windows-native.4` prototype/pending (not product CLOSED / not metal CLOSED / not claim=present); Cloud mocks do not close windows-native.4 | typed personalization.provider density/cursor/motion/full theme; Win7 bottom-row product; never claim=present |
| Accessibility | none | none | no `accessibility.provider` |
| System information | none | tip-true `SettingsSessionSystemInformation.readInformation` → `apply_system_information_inspect` soft leftover-attached to ACC `windows-native.38` prototype/pending (`system.info.read` claim=partial / human=partial; never claim=present) | no Fabric aggregate provider; Soft Ship park before suite EXIT 0 as metal; Cloud mocks do not close windows-native.38 |
| Files | `files.browse` | session leftover recorded: soft leftover-attach ACC `windows-native.9` via tip-true `FilesModel.js` `files.downloads` → `files.location.downloads` browse (visible `Start > Downloads; Superbar > Files > Downloads`; catalog `files.downloads.open` leftover-direct / claim=partial / human=partial; source tip-aligns off launch-files invent; READ-ONLY; no Fabric fingerprint invent; not product CLOSED / not metal CLOSED / not claim=present; `windows-native.9` stays prototype/pending; Cloud mocks do not close windows-native.9); `directory.create` (New folder LIVE); `files.entry.open` (Open LIVE, risk low); `files.entry.rename` (Rename LIVE, risk low, same-directory); `files.entry.copy` (Copy/Paste LIVE, risk low, in-app staging plus session clipboard, regular files and directories; folder copy CLOSED); session Trash/Restore/Empty (#76) via `omarchy-fabric-session-apply` (`files-entry-trash` / `files-trash-restore` / `files-trash-manage`); `files.entry.move` session Cut/Paste-after-cut, Fabric write plane CHANGES UNAVAILABLE under SHELL (`cutAuthorized=false`); `files.entry.delete` session Permanent Delete after confirm, Fabric write plane CHANGES UNAVAILABLE under SHELL (`deleteAuthorized=false`; permanent delete write plane exists but is not shell-authorizable); session Compress via `omarchy-fabric-session-apply` `files-archive-create` (visible `Files > Compress`; catalog `files.archive.create` leftover-direct / claim=partial; no Fabric SHELL LIVE archive writer; `windows-native.13` stays prototype/pending); session Extract via `omarchy-fabric-session-apply` `files-archive-extract` (visible `Files > Extract`; catalog `files.archive.extract` leftover-direct / claim=partial; non-colliding folder named after the archive beside the zip; no Fabric SHELL LIVE archive writer; no windows-native Extract row); session Properties via `omarchy-fabric-session-apply` `files-entry-properties` (visible `Files > right-click > Properties`; catalog `files.properties.read` leftover-direct / claim=partial; READ-ONLY name/type/size/modified/location; no Fabric SHELL LIVE Properties writer; `parity.properties` stays missing as product); session leftover recorded: soft leftover-attach ACC `windows-native.15` via tip-true `FilesSessionEject.ejectDevice` → `storage-removable-eject` (visible `Files > Devices > Eject`; catalog `storage.removable.eject` leftover-direct / claim=partial / human=partial; USB unmount+power-off / optical eject; `device.busy` refuse; no Fabric SHELL LIVE eject writer; not product CLOSED / not metal CLOSED / not claim=present; `windows-native.15` stays prototype/pending; Cloud mocks do not close windows-native.15; `ejectAuthorized=false`); session leftover recorded: soft leftover-attach ACC `windows-native.14` via tip-true `FilesSessionMount.mountVolume` → `storage-removable-mount` (visible `Files > Devices > Mount`; catalog `storage.removable.mount` leftover-direct / claim=partial / human=partial; USB/optical mount via udisksctl; `device.busy` refuse; no Fabric SHELL LIVE mount writer; not product CLOSED / not metal CLOSED / not claim=present; `windows-native.14` stays prototype/pending; Cloud mocks do not close windows-native.14; `mountAuthorized=false`); session leftover recorded: session Connect via `omarchy-fabric-session-apply` `sharing-smb-connect` (visible `Files > Connect to Server`; catalog `sharing.smb.connect` leftover-direct / claim=partial; guest/public `gio mount --anonymous` only; `share.auth-required` refuse; no Fabric SHELL LIVE SMB writer; session-UI leftover only; not product CLOSED; `windows-native.16` stays missing/pending; Cloud mocks do not close windows-native.16; `smbAuthorized=false`); session leftover recorded: Open a PDF via tip-true `files.entry.open` / `files-entry-open` → `/usr/bin/xdg-open` (visible `Files > PDF`; catalog `files.document.open` leftover-direct / claim=partial soft leftover-attached to `openEntry`; Catalog tip-align 2026-09-06 MUST_FIX: kind=`writer` effects=`launch` (not reader invent); consent=`implicit`; recovery tip-true openEntry → xdg-open launch (no read-only invent); shipped `application/pdf` → Evince; session-UI leftover-attach only; not product CLOSED; `windows-native.17` stays prototype/pending; Cloud mocks do not close windows-native.17; no Open With / MIME association UI invent); session leftover recorded: Edit a text file via tip-true `files.entry.open` / `files-entry-open` → `/usr/bin/xdg-open` (visible `Files > Text`; catalog `files.text.edit` leftover-direct / claim=partial soft leftover-attached to `openEntry`; Catalog tip-align 2026-09-06 MUST_FIX: kind=`writer` effects=`launch` (not mutating invent); consent=`implicit`; recovery tip-true openEntry → xdg-open launch (no Files inventory undo / no fingerprint invent); shipped `text/plain` → org.gnome.TextEditor not Neovim-as-default; session-UI leftover-attach only; not product CLOSED; `windows-native.18` stays prototype/pending; Cloud mocks do not close windows-native.18; no Open With / MIME association UI invent); Fabric `files.entry.trash` and `files.trash.restore` write planes, CHANGES UNAVAILABLE under SHELL (`trashAuthorized=false`); Fabric `files.trash.manage` write plane, CHANGES UNAVAILABLE under SHELL (`emptyBinAuthorized=false`) | Fabric Restore LIVE under SHELL, Fabric Empty Bin LIVE under SHELL, Recycle Bin is not product-complete, Fabric LIVE Delete, Fabric SHELL Cut (session Cut CLOSED by #77), OS clipboard residual OPEN after PR #60, OS clipboard residual OPEN after PR #64, OS clipboard residual CLOSED for Files copy-out/paste-in, desktop Recycle icon ships as Recycle Bin.desktop (files.trash), desktop context menu; Recycle Bin / Empty Bin LIVE residual OPEN after PR #63 (`files.trash.manage` write plane reachable; claim=partial; humanRoute planned empty); Recycle Bin is not product-complete; Explorer present; Files LIVE metal; Win7 visual |
...
- Default Programs = **four links** (Set defaults / Associate / AutoPlay / SPAD) — browser https + email mailto + per-MIME defaults in Settings > Default Programs ≠ `parity.default-programs` present; per-association defaults live (`parity.file-associations` stays missing until `files.associations.set` + applet).
...
- Accessibility: honest missing — **omit** provider-less jump destinations. System information: soft leftover-attach ACC `windows-native.38` to tip-true Settings > System information via `SettingsSessionSystemInformation.readInformation` → `apply_system_information_inspect` (`windows-native.38` prototype/pending; not claim=present; Cloud mocks do not close windows-native.38).
...
- **Behavioral exit (vagueness fix):** `windows-native.10–15,17–18` + `parity.explorer-this-pc` + Recycle place with trash/restore; MIME per-association defaults UI live in Settings > Default Programs (`files.associations.set` still planned); SMB connect UI; GUI txt editor; banner/handoff must match UI (no underclaim trash if offered).
...
Task Manager, Device Manager, Storage, Backup vs snapshots, Recovery, firmware, Troubleshooting Center. Phase 9 exit criteria still open. Live Administration inspect hosts today: Processes (`process.inspect` claim=partial CPU plane via FixedArgv `/usr/bin/ps` + unauthorized End Task OPEN leftover path), Services, Device Manager, Storage, Printers and scanners, Backup, Scheduled tasks, Troubleshooting, Firewall, User accounts. Inspect inventory ≠ product MMC present. No Task Manager present, no Device Manager present, no Services present, no End Task LIVE CONTROL (`terminationAuthorized=false`). `windows-native.26` stays prototype/pending; session leftover recorded / not product CLOSED / not metal CLOSED / not claim=present. Win7 Task Manager tabs stay reference: Applications | Processes | Services | Performance | Networking | Users.

...
- Default Programs = **four links** (Set defaults / Associate / AutoPlay / SPAD) — browser https + email mailto + per-MIME defaults in Settings Apps ≠ `parity.default-programs` present; per-association defaults live (`parity.file-associations` stays missing until `files.associations.set` + applet).
...
- **Behavioral exit (vagueness fix):** `windows-native.10–15,17–18` + `parity.explorer-this-pc` + Recycle place with trash/restore; MIME per-association defaults UI live in Settings Apps (`files.associations.set` still planned); SMB connect UI; GUI txt editor; banner/handoff must match UI (no underclaim trash if offered).

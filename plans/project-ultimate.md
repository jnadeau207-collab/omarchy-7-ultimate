...
| Files | `files.browse` | `directory.create` (New folder LIVE); `files.entry.open` (Open LIVE, risk low); `files.entry.rename` (Rename LIVE, risk low, same-directory); `files.entry.copy` (Copy/Paste LIVE, risk low, in-app staging plus session clipboard, regular files and directories; folder copy CLOSED); `files.entry.move` session Cut/Paste-after-cut, Fabric write plane CHANGES UNAVAILABLE under SHELL (`cutAuthorized=false`); `files.entry.delete` session Permanent Delete after confirm, Fabric write plane CHANGES UNAVAILABLE under SHELL (`deleteAuthorized=false`; permanent delete write plane exists but is not shell-authorizable); `files.entry.trash` and `files.trash.restore` write planes, CHANGES UNAVAILABLE under SHELL (`trashAuthorized=false`); `files.trash.manage` write plane, CHANGES UNAVAILABLE under SHELL (`emptyBinAuthorized=false`) | Restore UI (do not invent Restore LIVE), Empty Bin LIVE, Fabric LIVE Delete, Fabric LIVE Cut, OS clipboard residual OPEN after PR #60, OS clipboard residual OPEN after PR #64, OS clipboard residual CLOSED for Files copy-out/paste-in, desktop Recycle icon ships as Recycle Bin.desktop (files.trash), desktop context menu; Recycle Bin / Empty Bin LIVE residual OPEN after PR #63 (`files.trash.manage` write plane reachable; claim=partial; humanRoute planned empty); Recycle Bin is not product-complete |

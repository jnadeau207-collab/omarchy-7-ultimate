...
| Files | `files.browse` | `directory.create` (New folder LIVE); `files.entry.open` (Open LIVE, risk low); `files.entry.rename` (Rename LIVE, risk low, same-directory); `files.entry.copy` (Copy/Paste LIVE, risk low, in-app staging, regular files and directories; folder copy CLOSED); `files.entry.move` write plane, CHANGES UNAVAILABLE under SHELL (`cutAuthorized=false`); `files.entry.delete` write plane, CHANGES UNAVAILABLE under SHELL (`deleteAuthorized=false`; permanent delete write plane exists but is not shell-authorizable); `files.entry.trash` and `files.trash.restore` write planes, CHANGES UNAVAILABLE under SHELL (`trashAuthorized=false`) | Restore UI (do not invent Restore LIVE), empty Recycle Bin, LIVE Delete, LIVE Cut, OS clipboard residual OPEN after PR #60, desktop Recycle icon, desktop context menu; Recycle Bin / `files.trash.manage` residual OPEN after PR #60 (`availability.claim=missing`); Recycle Bin is not product-complete |

...
| **6** | Files and defaults | Dolphin/This PC, desktop files, graphical txt editor, MIME defaults, removable media, SMB. | **Not closed.** Product Files prototype (parity: rename and in-app copy/paste landed as claim=partial; dest-scoped move write plane landed at consequential with `cutAuthorized=false`; OS clipboard/folder copy/column resize/Explorer present absent). |

# Phase Bindings — Win7 Ultimate ground-truth splice for `plans/project-ultimate.md`

**Usage:** Under each Phase 0–11 heading in `plans/project-ultimate.md`, add a subsection **Win7 Ultimate binding specs** with the bullets below (keep links; do not dump full ground-truth bodies into the plan).  
**Caption BINDING LOCK (all phases that touch chrome):** visual restored top NC = **30** = `SM_CYFRAME(4)+SM_CYCAPTION(22)+SM_CXPADDEDBORDER(4)`; `SM_CYCAPTION=22` is metric band only — **never** ship a 22 px title bar; buttons visual **29×20 / 27×20 / 49×20**; cluster width **105**; prefer live `DWMWA_CAPTION_BUTTON_BOUNDS` for proof.  
**Doctrine:** Product **REJECTED**; anti-invent checklist in `plans/WIN7_SPEC_INDEX.md` + `fleet-doctrine-gaps.md`.  
**Ground-truth root:** `plans/win7-ultimate-ground-truth/`
...
- Gallery / tokens: Ultimate 2026 language is allowed; do not redefine Win7 geometry from Omarchy tokens (`01-window-chrome.md` § Omarchy freeze).
...
- Links: `WIN7_SPEC_INDEX.md`, `07-interaction-grammar.md` (global metrics), `01-window-chrome.md`
...
- Links: `01-window-chrome.md`, `03-superbar-taskbar.md` (Show Desktop/peeks), `07-interaction-grammar.md` (Aero + hotkeys)
...
- Not a visual Win7 phase; consent/progress sheets follow `07-interaction-grammar.md` dialog/progress grammar (OK/Cancel order, consequential state Rule 6).
...
- Links: `07-interaction-grammar.md`, `02-start-menu.md` (Agent Center as Start place = 2026 delta)
...
- Hit targets / focus rings / motion: `07-interaction-grammar.md` (hover 400 ms, drag 4 px, dialog 75×23 @ 96).
...
- Links: `01-window-chrome.md`, `07-interaction-grammar.md`, `03-superbar-taskbar.md` (40 vs 48 height delta)
...
- **Start:** two-pane; BINDING right-pane order User→Documents→Pictures→Music→Games→Computer→Control Panel→Devices and Printers→Default Programs→Help; power flyout Switch user/Log off/Lock/Restart/Sleep/Hibernate; All Programs **in-pane** with Back; search placeholder “Search programs and files”; orb 54×54 states (`02-start-menu.md`). Omarchy 720×640 / Settings/Agent Center places = documented deltas.
- **Superbar:** height 40 (Win7) vs product 48; combine modes; Jump List structure; left-click minimize-active; Shift/middle new instance; tray Action Center/Volume/Network/Power/Clock; Show Desktop (`03-superbar-taskbar.md`).
- Desktop icons + context menu order View/Sort/Refresh/Paste/New/Screen resolution/Gadgets/Personalize (`04-explorer-dialogs.md`).
...
- Links: `02-start-menu.md`, `03-superbar-taskbar.md`, `04-explorer-dialogs.md` (desktop), `07-interaction-grammar.md`
...
- Map Settings pages to Win7 CP applets in `05-control-panel.md` (55 items) + `06-settings-defaults-admin-media.md` Personalization / Sound / Display / Power / Default Programs.
...
- Links: `05-control-panel.md`, `06-settings-defaults-admin-media.md`, `07-interaction-grammar.md`
...
- Explorer chrome: command bar (not ribbon); nav Favorites→Libraries→Homegroup→Computer→Network; Libraries defaults; views Extra Large…Content; Computer groups + capacity bars; Recycle Bin behaviors (`04-explorer-dialogs.md`).
...
- AutoPlay from Default Programs child (`06-…`).
- Links: `04-explorer-dialogs.md`, `06-settings-defaults-admin-media.md`, `05-control-panel.md` (Folder Options)
...
- Media player product may reference WMP 12 chrome jobs (`06-…`) without clone mandate.
- Links: `05-control-panel.md` (Programs and Features), `06-settings-defaults-admin-media.md`
...
- Default Programs / AutoPlay remain human path for “which program opens this” (`06-…`).
- Links: `06-settings-defaults-admin-media.md`, `05-control-panel.md`
...
- Each noun → job id: Task Manager (`parity.task-manager`), Device Manager, Disk Management, Services, Task Scheduler, Event Viewer (≠ toast history), Firewall, User Accounts, Backup, Recovery, Troubleshooting (`05-control-panel.md`, `06-…` MMC trees).
...
- Links: `06-settings-defaults-admin-media.md`, `05-control-panel.md`, `fleet-doctrine-gaps` Administration rows
...
- Dialog/progress grammar from `07-interaction-grammar.md`; no-bullshit install Rule.
- Links: `07-interaction-grammar.md`, `02-start-menu.md` (power), `05-control-panel.md` (Getting Started analog only)
...
- Apply full `07-interaction-grammar.md`: hover/drag/menus/dialogs/progress/balloons/Aero gestures/DPI/themes/sounds/cursors/Win+ hotkeys.
...
- Links: `07-interaction-grammar.md`, all `01`–`06` ground-truth files, `WIN7_SPEC_INDEX.md`

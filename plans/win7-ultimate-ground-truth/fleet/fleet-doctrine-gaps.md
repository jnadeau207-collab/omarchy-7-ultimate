...
- Catalog soft invent: closed. Catalog writer is `defaults.protocol.set` on `defaults.provider` (availability.claim=partial). `parity.default-programs` sourceStatus is prototype with PARITY Default Programs. No `apps.provider` present. MIME / `files.associations.set` / `parity.file-associations` stay missing. Product REJECTED.
- Catalog soft invent (`files.folder.create`): closed. Catalog writer is `files.directory.create` on `files.provider` (availability.claim=partial). Live Files drives `directory.create` via `FilesApplication.createFolder`. No Nautilus source. `parity.explorer-this-pc` and `windows-native.10` name `files.directory.create`. Rename / copy / Restore stay unraised. `files.trash.manage` stays missing. Product REJECTED.
- Catalog soft invent (missing live `files.entry.trash` writer): closed. Catalog writer is `files.entry.trash` on `files.provider` (provider.state=present, availability.claim=partial, consent high-risk / consequential). Live Files drives `entry.trash` via `FilesApplication.trashEntry`. `parity.explorer-this-pc` and `parity.desktop-icons-wallpaper-context-menu-recycle` name `files.entry.trash` without walking claim→present. No windows-native delete-to-recycle row exists to retarget. `files.trash.manage` stays missing/planned. Restore stays honest-unavailable. Rename / copy stay unraised. Product REJECTED.
- Catalog soft invent (missing Admin `process.termination.plan` writer): closed. Catalog writer is `process.termination.plan` on `process.provider` (provider.state=present, availability.claim=missing, consent high-risk / consequential). Administration wires `termination.plan` via `AdministrationApplication.endTask` with `terminationAuthorized=false` (End Task stays hidden). `parity.task-manager` names `process.termination.plan` without walking claim→present. No Task Manager product. No End Task LIVE CONTROL. `apps.startup.disable` stays missing. Product REJECTED.
- Catalog soft invent (leftover `processes.inspect` empty-path / `processes.provider` / legacy-direct): closed. Leftover `processes.inspect` removed. Honest reader is `process.inspect` on `process.provider` (provider.state=present, availability.claim=missing). `parity.task-manager` and `windows-native.26` name `process.inspect` without walking claim→present. humanRoute stays planned Administration empty-path. No Task Manager product. No End Task LIVE CONTROL. `process.termination.plan` stays missing. `apps.startup.disable` stays missing. Product REJECTED.

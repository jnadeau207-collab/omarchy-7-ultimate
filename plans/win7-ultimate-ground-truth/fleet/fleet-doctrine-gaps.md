...
- Settings/Admin/Files catalog soft invents: locked closed (source-oracle). Leftover invent ids `processes.inspect`, `files.folder.create`, and `apps.defaults.set` are absent. No `apps.provider` present writers. No Task Manager surface on `process.*` routes. `resources.inspect` stays honest-missing (Resource Monitor empty path; not Task Manager Performance; not btop). Live Settings/Admin/Files writers + process inspect/termination:
  ...
  | Settings Power profile (`profile.set`) write plane, not Settings LIVE; Superbar QS leftover unverified on metal | `power.profile.set` | partial |
  ...
  Do not invent MIME, Restore LIVE, `files.trash.manage`, LIVE Trash under SHELL, Software Center LIVE, Task Manager present, or End Task LIVE CONTROL. Rename / copy stay unraised. Settings badge METAL_HEAD residual stays OPEN. Product REJECTED.
...
- Semantics.text / authored English leftovers (Phase 3 / `b91b77d4` cluster). Source-oracle wrap closed on this Cloud pass for named product chrome: Settings/Admin `LIVE PANEL` / `PARTIAL LIVE CONTROL` / `CHANGES UNAVAILABLE` / `LIVE CONTROL` badges via `Ui.Badge` semanticProfile; Exact resource prefix; display-bound notice; Open tooltip a11y via `Ui.Button`; Files Reconnect/Try again; hosted-panel load failure; Personalization Theme packs/Wallpaper; writer-pane Accessible.name; Software/Compatibility Reconnect/Try again via `Ui.Button` semanticProfile + host productProfile; Files Explorer Aero chrome (command bar, address bar, view labels) via Semantics.text / productProfile. Path crumbs, entry titles, and type cells stay literal. Source-oracle leftover 2026-09-02 Cloud (no compositor): `test/shell.d/design-system-consumer-test.sh` EXIT 0; `test/shell.d/domain-product-static-test.sh` EXIT 0; `test/shell.d/semantic-ui-contract-test.sh` EXIT 0; `test/shell.d/administration-product-test.sh` EXIT 0. Residual (do not invent): no grim/screenshot + SHA on this Cloud VM — **pixel leftover still open pending METAL_HEAD**. DesktopIcons argv→execArgv landed in tree (PR #5). Metal leftover 2026-09-02T04:18:31-04:00 on omarchy: `test/shell.d/launch-argv-test.sh` EXIT 0; comma-join `uwsm-app -- /usr/bin/cursor,--password-store=...` still errors as expected; `uwsm-app -- /home/jesse/.local/bin/cursor-wayland --version` OK (3.16.29). Product REJECTED.
- Settings coverage badges (PR #6 / work `e7eded14`, Semantics wrap this pass): PARTIAL LIVE CONTROL vs CHANGES UNAVAILABLE stay route-honest in `SettingsModel.coverageBadge` and are pinned in `ultimate-settings-product-test.sh`. AuthorityFooter names LIVE writers. Source-oracle leftover 2026-09-02 Cloud (no compositor): `test/shell.d/ultimate-settings-product-test.sh` EXIT 0; `test/shell.d/domain-product-static-test.sh` EXIT 0; `test/shell.d/design-system-consumer-test.sh` EXIT 0. **Settings badge pixel leftover still open pending METAL_HEAD**; this Cloud VM has no Omarchy/Quickshell compositor and produced no grim + SHA. Do not claim pixel leftover or METAL_HEAD. Product REJECTED.
- Settings live inspect refresh (`HANDOFF_STATE_DOMAIN_WRITES` UI defect): closed local-writer `succeeded` re-query (`refreshAfterSuccessfulWriter`) and host `surfaceBecameActive` re-query (shown / unminimized / later IPC activate). No `events.subscribe`; Fabric has no hardware-key resource-change topic, and Settings still forbids `events.*`. Residual: hardware-key or other out-of-band mutation while the open Settings page stays focused and already visible. Source-oracle leftover 2026-09-02 Cloud: `test/shell.d/ultimate-settings-product-test.sh` EXIT 0. Not metal leftover. Settings badge METAL_HEAD residual stays OPEN. Product REJECTED.
- Administration / Files live inspect refresh (Settings `refreshWhenSurfaceVisible` twin): closed host `surfaceBecameActive` re-query of the open route via `refreshWhenSurfaceVisible` wrapping existing `controller.refresh()` (shown / unminimized / later IPC activate). Skips offline, catalog-loading, loading, and QML `operationBusy`. No `events.subscribe`; no polling daemon. Residual: out-of-band mutation while the open page stays focused and already visible. Source-oracle leftover 2026-09-02 Cloud: `test/shell.d/administration-product-test.sh` EXIT 0; `test/shell.d/ultimate-files-create-test.sh` EXIT 0. Not metal leftover. Settings badge METAL_HEAD residual stays OPEN. Product REJECTED.
- Software / Compatibility live inspect refresh (Settings/Admin/Files `refreshWhenSurfaceVisible` twin): closed host `surfaceBecameActive` re-query of the open route via `refreshWhenSurfaceVisible` wrapping existing `controller.refresh()` (shown / unminimized / later IPC activate). Skips offline, catalog-loading, loading, and QML `busy` (these surfaces have no writers, so no `operationBusy`). No `events.subscribe`; no polling daemon. Residual: out-of-band mutation while the open page stays focused and already visible. Do not invent package LIVE mutation. Source-oracle leftover 2026-09-02 Cloud: `test/shell.d/software-compatibility-product-test.sh` EXIT 0. Not metal leftover. Settings badge METAL_HEAD residual stays OPEN. Product REJECTED.
- Files `entry.trash` write plane (PR #8): widen v1 directory family, not a parallel Trash family; deriver reads scoped `currentState.value`; risk stays `consequential` (not the same class as `directory.create` `low`; SHELL cannot hold a standing grant). Files Trash UI honesty: controls gated `trashAuthorized=false`; no doomed shell preflight; New folder stays LIVE. Restore stays honest-unavailable. No `files.trash.manage`. Recycle Bin not product-complete. Source-oracle leftover 2026-09-02: failing `operation.intent-build-failed`/`KeyError`; after unwrap, leftover stdout is `entry.trash preflight: OK` / `entry.trash risk: consequential` / `entry.trash SHELL grant: FAIL grant.shell-consequential` / `trash.restore preflight: operation.definition-unavailable`. Commands actually run (command + EXIT) are cited in PR #8. **Not** metal leftover. Product REJECTED.
...
- Superbar / Quick Settings Power leftover after PR #31 (`62372edf9e62`): Settings Power LIVE stays refused. Catalog keeps `power.profile.set` on Quick Settings / `shell/plugins/panels/power/Panel.qml` / `legacy-direct`. Heritage panel still offers profile buttons via Process/`omarchy-powerprofiles-set` (not the fabric durable plane). Placement from unit files / autostart / oomd note: Hyprland is `session.slice` / `wayland-wm@hyprland.desktop.service`; autostart is `hl.exec_cmd("omarchy-launch-shell")` not `uwsm-app`; Quickshell is not a recorded login `session-N.scope`. Distinct from fabric `app.slice`. Whether that Process child satisfies polkit `allow_active` is **unverified on metal**. Do not honesty-gate the heritage panel without that leftover. Do not invent Settings Power LIVE, a broad polkit grant, or METAL_HEAD closed. Soften any claim that the leftover is a proven working human route. Source-oracle leftover 2026-09-02 Cloud: `test/shell.d/capability-catalog-test.sh`; `test/shell.d/ultimate-settings-writers-test.sh`; `test/shell.d/powerprofiles-set-test.sh`. Not metal leftover. METAL_HEAD OPEN. Product REJECTED.

...
- Files banner / `HANDOFF_WRITERS`: closed helper-only / schema-blocker stale narrative for `entry.trash` after PR #8 v1 directory family widen. LIVE: Files banner names New folder and Trash as live writers; `directory.create` risk `low`; `entry.trash` write plane reachable, risk `consequential`; SHELL refused at `grant.shell-consequential`. Residual: Restore UI honest-unavailable (no `.trashinfo` at real preflight; `trash.restore` `operation.definition-unavailable`); no `files.trash.manage`; no Empty Recycle; Recycle Bin not product-complete. Do not invent Restore LIVE. Source-oracle leftover 2026-09-02 Cloud: `test/shell.d/domain-product-static-test.sh` EXIT 0; `test/shell.d/ultimate-files-create-test.sh` EXIT 0. Not metal leftover. Product REJECTED.
- Settings/Admin/Files catalog soft invents: locked closed (source-oracle). Leftover invent ids `processes.inspect`, `files.folder.create`, and `apps.defaults.set` are absent. No `apps.provider` present writers. No Task Manager surface on `process.*` routes. `resources.inspect` stays honest-missing (Resource Monitor empty path; not Task Manager Performance; not btop). Live Settings/Admin/Files writers + process inspect/termination:
  ...
  | Files Trash (`entry.trash`) | `files.entry.trash` | partial |
  ...
  Do not invent MIME, Restore LIVE, `files.trash.manage`, Software Center LIVE, Task Manager present, or End Task LIVE CONTROL. Rename / copy stay unraised. Settings badge METAL_HEAD residual stays OPEN. Product REJECTED.

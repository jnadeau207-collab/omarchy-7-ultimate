...
- Semantics.text / authored English leftovers (Phase 3 / `b91b77d4` cluster). Source-oracle wrap closed on this Cloud pass for named product chrome: Settings/Admin `LIVE PANEL` / `PARTIAL LIVE CONTROL` / `CHANGES UNAVAILABLE` / `LIVE CONTROL` badges via `Ui.Badge` semanticProfile; Exact resource prefix; display-bound notice; Open tooltip a11y via `Ui.Button`; Files Reconnect/Try again; hosted-panel load failure; Personalization Theme packs/Wallpaper; writer-pane Accessible.name. Source-oracle leftover 2026-09-02 Cloud (no compositor): `test/shell.d/design-system-consumer-test.sh` EXIT 0; `test/shell.d/semantic-ui-contract-test.sh` EXIT 0; `test/shell.d/administration-product-test.sh` EXIT 0. Residual (do not invent): Files Explorer Aero chrome (command bar, address bar, view labels) still mostly raw English; Software/Compatibility Reconnect buttons still lack semanticProfile; no grim/screenshot + SHA on this Cloud VM — **pixel leftover still open pending METAL_HEAD**. DesktopIcons argv→execArgv landed in tree (PR #5). Metal leftover 2026-09-02T04:18:31-04:00 on omarchy: `test/shell.d/launch-argv-test.sh` EXIT 0; comma-join `uwsm-app -- /usr/bin/cursor,--password-store=...` still errors as expected; `uwsm-app -- /home/jesse/.local/bin/cursor-wayland --version` OK (3.16.29). Product REJECTED.

...
| 51 | Windows Update | Yes | Settings → Update | `parity.update` prototype; `update.install` session apply leftover; `update.history.read` session log leftover | Session apply + history read; Fabric `system.update` inspect-only; rows 28/29 pending |

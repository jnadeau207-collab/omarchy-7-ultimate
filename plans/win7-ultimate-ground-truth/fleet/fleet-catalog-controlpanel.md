...
| Explorer Open | Files Open / `files.entry.open` launch plane is reachable (claim=partial; risk low; SHELL-grantable; visible `Files > Open`); PDF soft leftover-attach via `files.document.open` leftover-direct claim=partial visible `Files > PDF` reuses that plane; Catalog tip-align 2026-09-06 MUST_FIX: kind=`writer` effects=`launch` (not reader invent); recovery tip-true openEntry → xdg-open launch (no read-only invent); `windows-native.17` stays prototype/pending; Cloud mocks do not close windows-native.17; text-edit soft leftover-attach via `files.text.edit` leftover-direct claim=partial visible `Files > Text` reuses that plane with tip-true `text/plain` → org.gnome.TextEditor (not Neovim-as-default); Catalog tip-align 2026-09-06 MUST_FIX: kind=`writer` effects=`launch` (not mutating invent); consent=`implicit`; recovery tip-true openEntry → xdg-open launch (no Files inventory undo / no fingerprint invent); `windows-native.18` stays prototype/pending; Cloud mocks do not close windows-native.18; no Open With / MIME association UI |

...
**Product honesty after #35–#39:** Administration inspect readers are **visible** on live Administration titles after #36; other Admin inspect `availability.claim` stays **missing** while `process.inspect` is soft leftover-attached claim=**partial** / human=**partial** for the FixedArgv `/usr/bin/ps` CPU plane; Phase 9 exit still open; inspect ≠ MMC product present. `process.inspect` is **visible** on `Administration > Processes`; honest-unavailable as Task Manager product; `windows-native.26` stays prototype/pending; End Task unauthorized (`terminationAuthorized=false`; OPEN leftover); do not invent Task Manager present / End Task LIVE / claim=present. Writers (`firewall.manage`, `backup.manage`, start/stop, …) stay planned/unavailable. Default Programs writers are `defaults.protocol.set` (browser and mailto) and `defaults.mime.set` (partial MIME rows LIVE on Settings > Default Programs); Default Programs applet still missing; `files.associations.set` stays missing/planned MIME. Catalog humanRoutes already published visible; residual = product MMC / mutation / Task Manager present still missing, not catalog planned-empty. Catalog caught up for Admin inspect visibility (#35–#36). Product remains **REJECTED**. Do not invent Accessibility panel. Software Center missing as product. Settings Power LIVE refused. Settings coverage-badge PIXEL leftover CLOSED on `d3f4841a`. QS Power METAL_HEAD OPEN (metal FAIL on tip `20484de6`; leftover was unverified on metal after PR #31). Do not claim the heritage QS Power panel works on this metal.
...
| Recycle Bin | Files Trash routes / `files.trash.*` plane; Desktop Recycle icon is the shipped `Recycle Bin.desktop` shortcut (`omarchy-launch-files --source desktop files.trash`); `files.trash.restore` write plane is reachable (claim=partial; humanRoute planned empty); session Restore is visible `Files > Restore`; `files.trash.manage` write plane is reachable (claim=partial; humanRoute planned empty; `emptyBinAuthorized=false`); session Empty Recycle Bin is visible `Files > Empty Recycle Bin`; Recycle Bin / Empty Bin LIVE residual OPEN after PR #63 |
| Explorer Open | Files Open / `files.entry.open` launch plane is reachable (claim=partial; risk low; SHELL-grantable; visible `Files > Open`); PDF soft leftover-attach via `files.document.open` leftover-direct claim=partial visible `Files > PDF` reuses that plane; `windows-native.17` stays prototype/pending; Cloud mocks do not close windows-native.17; text-edit soft leftover-attach via `files.text.edit` leftover-direct claim=partial visible `Files > Text` reuses that plane with tip-true `text/plain` → org.gnome.TextEditor (not Neovim-as-default); `windows-native.18` stays prototype/pending; Cloud mocks do not close windows-native.18; no Open With / MIME association UI |
...
| Explorer Copy and Paste | Files Copy and Paste / `files.entry.copy` write plane is reachable (claim=partial; risk low; SHELL-grantable; visible `Files > Copy and Paste`; `copyAuthorized=true`); regular files and directories; folder copy CLOSED; in-app staging plus session clipboard copy-out/paste-in; OS clipboard residual OPEN after PR #60; OS clipboard residual OPEN after PR #64; OS clipboard residual CLOSED for Files copy-out/paste-in |
| Explorer Cut and Paste | Files cut/move / `files.entry.move` write plane is reachable (claim=partial; risk consequential; SHELL refused at `grant.shell-consequential`; humanRoute visible `Files > Cut`; `cutAuthorized=false`); session Cut/Paste-after-cut for regular files; dest-scoped identity; no Fabric LIVE Cut; folder copy CLOSED via `files.entry.copy` |
| Explorer Permanent Delete | Files permanent delete / `files.entry.delete` write plane is reachable (claim=partial; risk consequential; SHELL refused at `grant.shell-consequential`; humanRoute visible `Files > Permanently delete`; `deleteAuthorized=false`); session Permanent Delete after confirm for regular files and empty directories; identity-bound; permanent delete write plane exists but is not shell-authorizable; no Fabric LIVE Delete or Empty Bin LIVE; Recycle Bin / Empty Bin LIVE residual OPEN after PR #63 |
...
| System | `Microsoft.System` | Settings → System information (session read leftover); Admin/system info missing as product | `system.info.read` leftover `legacy-direct` claim=partial visible Settings > System information; `windows-native.38` prototype/pending | session `system-information-inspect`; no Fabric `system-information.provider` durable LIVE |
| Windows Update | `Microsoft.WindowsUpdate` | Settings → Update | `parity.update` prototype; `update.inspect` partial; `update.install` leftover session apply (claim=partial, visible Settings > Update); `update.history.read` leftover session log read (claim=partial, visible Settings > Update; soft leftover-attach ACC `windows-native.29` via `SettingsSessionUpdate.qml` `readHistory`); Fabric `system.update` inspect-only; restart/reboot writers pending; `windows-native.28` pending; `windows-native.29` prototype/pending | `update.provider` |
...
| Task Manager (often reached via Ctrl+Shift+Esc, not only Admin Tools) | Administration > Processes (visible CPU inspect; do not invent Task Manager present) | `process.inspect` **visible** on `Administration > Processes`; claim=**partial** / human=**partial** (FixedArgv `/usr/bin/ps` `pcpu`); honest-unavailable as Task Manager product; `windows-native.26` stays prototype/pending; End Task unauthorized (`terminationAuthorized=false`; OPEN leftover); do not invent Task Manager present / End Task LIVE / claim=present; `parity.task-manager` stays missing as product |
...
| Sound | Settings → Sound; Superbar/QS | `parity.sound` prototype; `audio.inspect`; Settings volume `audio.volume.set`; session mute/default soft leftover-attach `audio.output.manage` leftover-direct claim=partial; session audio restart soft leftover-attach `troubleshooting.audio.run` leftover-direct claim=partial (`windows-native.39` prototype/pending; ports/full wizard OPEN leftover) | `audio.provider` / session-apply |
...
| Taskbar and Start Menu | Superbar + Start (chrome, not CP applet) | `parity.superbar-taskbar` / `parity.start` prototype; `power.shutdown` leftover partial soft leftover-attach ACC `windows-native.40` via `Start.shutdown` → `omarchy-system-shutdown` (visible Start > Shut down; never claim=present) | shell plugins; `windows-native.40` prototype/pending |
...
| 43 | Taskbar and Start Menu | Yes | Superbar + Start | `parity.superbar-taskbar` / `parity.start`; soft leftover-attach wn.40 Shut down via `Start.shutdown` | Prototype/pending wn.40; never claim=present |
...
| 51 | Windows Update | Yes | Settings → Update | `parity.update` prototype; `update.install` session apply leftover; `update.history.read` session log leftover soft leftover-attached to wn.29 | Session apply + history read (`readHistory`); Fabric `system.update` inspect-only; row 28 pending; row 29 prototype/pending |
...
| --- | --- | --- |
| Recycle Bin | Files Trash | `files.trash.restore` write plane is reachable (claim=partial; humanRoute planned empty); session Restore is visible `Files > Restore`; `files.trash.manage` write plane is reachable (claim=partial; humanRoute planned empty); session Empty Recycle Bin is visible `Files > Empty Recycle Bin`; Empty Bin LIVE **MUST_FIX** vs product-complete; do not invent Fabric Restore LIVE or Empty Bin LIVE under SHELL |
| Task Manager | Administration > Processes | `process.inspect` visible; claim=partial (CPU FixedArgv `/usr/bin/ps`); honest-unavailable as Task Manager product; End Task unauthorized (`terminationAuthorized=false`; OPEN leftover); do not invent Task Manager present / End Task LIVE / claim=present |
...
| `administration.processes.overview` | Task Manager | `process.inspect` **visible** on `Administration > Processes`; claim=**partial** (CPU FixedArgv `/usr/bin/ps`); honest-unavailable as Task Manager product; `windows-native.26` stays prototype/pending; End Task unauthorized (`terminationAuthorized=false`; OPEN leftover); do not invent Task Manager present / End Task LIVE / claim=present |
...
1. **Recycle Bin product vs Empty Bin LIVE / Recycle product-complete.** `files.trash.restore` write plane is reachable (claim=partial; humanRoute planned empty). Session Restore is visible `Files > Restore`. `files.trash.manage` write plane is reachable (claim=partial; humanRoute planned empty; `emptyBinAuthorized=false`). Session Empty Recycle Bin is visible `Files > Empty Recycle Bin`. Recycle Bin / Empty Bin LIVE residual OPEN after PR #63. Residual stays OPEN as product-complete. Do not invent Fabric Restore LIVE or a LIVE Empty Bin under SHELL. OS clipboard residual OPEN after PR #60. OS clipboard residual OPEN after PR #64. Folder copy CLOSED.  
2. **Task Manager present / End Task LIVE / product MMC still missing.** Catalog `process.inspect` is visible on `Administration > Processes`; claim=partial soft leftover-attached for CPU inspect; honest-unavailable as Task Manager product; `windows-native.26` stays prototype/pending; End Task unauthorized (`terminationAuthorized=false`; OPEN leftover). Do not invent Task Manager present / End Task LIVE / claim=present.  
...
4. **Do not invent MIME Default Programs LIVE** — MIME / Default Programs association UI residual OPEN after PR #62. partial MIME rows LIVE on Settings > Default Programs; Default Programs applet still missing. `files.associations.set` stays missing/planned MIME until Settings + catalog + PARITY agree. Browser and mailto writer is `defaults.protocol.set`. Do not invent `apps.provider` present.  
...
| Honest missing | Accessibility page | Catalog empty paths; Accessibility panel missing as product; System information is session leftover not product CLOSED |
...
| Residual honesty (product MMC / mutation still missing) | Recycle Bin vs Empty Bin LIVE; Task Manager present / End Task LIVE; MIME Default Programs; writers planned/unavailable; Agent Center claim stays prototype (do not invent claim=`present`); OS clipboard residual OPEN after PR #60; OS clipboard residual OPEN after PR #64; MIME / Default Programs association UI residual OPEN after PR #62 | Catalog caught up for Admin inspect visibility (#35–#36); Default Programs writers are `defaults.protocol.set` (browser and mailto LIVE) and `defaults.mime.set`; partial MIME rows LIVE on Settings > Default Programs; Default Programs applet still missing; `files.trash.restore` write plane is reachable (claim=partial; humanRoute planned empty); session Restore is visible `Files > Restore`; `files.trash.manage` write plane is reachable (claim=partial; humanRoute planned empty); session Empty Recycle Bin is visible `Files > Empty Recycle Bin`; Recycle Bin / Empty Bin LIVE residual OPEN after PR #63; folder copy CLOSED; `parity.agent-center` claim matches sourceStatus/PARITY prototype |

---

## 9. Suggested next catalog work (for Lead / Doctrine, not auto-executed)

1. Keep the **Default Programs** applet missing. Writers are `defaults.protocol.set` (browser and mailto LIVE) and `defaults.mime.set` (partial MIME rows LIVE on Settings > Default Programs). Do not invent `apps.provider` present or Default Programs product.  

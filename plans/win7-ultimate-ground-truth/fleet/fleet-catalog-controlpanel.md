...
| Power Options | `Microsoft.PowerOptions` | Settings → Power; Superbar/QS Power | `parity.power-options` prototype; `power.inspect`; `power.profile.set` write plane not Settings LIVE (polkit `app.slice`) | `power.provider` (session_operable=False) |

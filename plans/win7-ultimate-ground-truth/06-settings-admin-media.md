...
| Default Programs + file/protocol associations | **Phase 5** Apps/defaults + **Phase 6** MIME defaults | Default Programs, File associations | Phase 5/6 exit criteria still open. Live host today: Settings → Apps `defaults.inspect` + `defaults.protocol.set` (browser and mailto). `defaults.mime.set` write plane is reachable; `association.inspect` is published for durable apply re-read; partial MIME rows LIVE on Settings > Apps; Default Programs applet still missing. `files.associations.set` stays missing/planned MIME. MIME / Default Programs association UI residual OPEN after PR #62. Do not invent Default Programs product. |
...
|----------|------------|----------------------|
| Make Chrome default browser | Default Programs → Set your default programs → Chrome → Set this program as default | Phase 5 exit still open; live host today is Settings → Apps `defaults.protocol.set` (browser and mailto); Default Programs applet still missing |
| Open .pdf with Okular only | Associate… → `.pdf` → Change program | Phase 6 exit still open; `defaults.mime.set` write plane is reachable; `association.inspect` is published for durable apply re-read; partial MIME rows LIVE on Settings > Apps; Default Programs applet still missing. `files.associations.set` stays missing/planned MIME. MIME / Default Programs association UI residual OPEN after PR #62. Do not invent Default Programs product. |
...
| Default Programs | Apps (defaults) — must expose the **four-job** IA, not a single dropdown. Live host today is Settings > Apps `defaults.protocol.set` (browser and mailto) plus partial MIME rows. Default Programs applet still missing. |

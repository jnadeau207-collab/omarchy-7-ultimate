...
| Open .pdf with Okular only | Associate… → `.pdf` → Change program | Phase 6 exit still open; `defaults.mime.set` write plane is reachable; Settings does not offer MIME LIVE CONTROL. `files.associations.set` stays missing/planned MIME. Do not invent MIME UI. |

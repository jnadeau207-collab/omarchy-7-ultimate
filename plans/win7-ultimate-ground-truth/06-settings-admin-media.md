...
| Default Programs + file/protocol associations | **Phase 5** Apps/defaults + **Phase 6** MIME defaults | Default Programs, File associations | Phase 5/6 exit criteria still open. Live host today: Settings → Apps `defaults.inspect` + `defaults.protocol.set` (browser LIVE only). `defaults.mime.set` write plane is reachable; `association.inspect` is published for durable apply re-read; Settings does not offer MIME LIVE CONTROL. `files.associations.set` stays missing/planned MIME. MIME / Default Programs association UI residual OPEN after PR #62. Do not invent MIME UI. |
...
| Open .pdf with Okular only | Associate… → `.pdf` → Change program | Phase 6 exit still open; `defaults.mime.set` write plane is reachable; `association.inspect` is published for durable apply re-read; Settings does not offer MIME LIVE CONTROL. `files.associations.set` stays missing/planned MIME. MIME / Default Programs association UI residual OPEN after PR #62. Do not invent MIME UI. |

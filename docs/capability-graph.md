...
The checked inventory contains 137 capability descriptors. WindowService contributes 39 writers: 21 names are in the prototype `CapabilityBroker.windowVerbs` catalog and 18 remain public legacy-direct writers. Those 21 broker names stay `availability.agent=unavailable` until a Fabric principal owns the verb. A caller-string allowlist is not Fabric ownership. Five additional public WindowService methods are readers, producing an exact 44-method service inventory. Twenty-two typed leaf providers add honest read-only inventory capabilities in builtin factory order; these providers do not make the corresponding managed-agent routes available.

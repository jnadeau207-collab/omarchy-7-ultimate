...
- `org.omarchy.Settings` hosts the image picker on Personalization. That hosted surface still runs `Process` / `execDetached` / `hyprctl` / `bash -c`. Display, Sound, Network, Bluetooth, and Power read the existing Fabric `display.inspect` / `audio.inspect` / `network.inspect` / `bluetooth.inspect` / `power.inspect` inventories (connector/mode/scale; sinks/mute/channel volume; Wi-Fi radio/interfaces; controller/devices; AC/profile/battery) instead of embedding the Process monitor, audio, nmcli, bluetoothctl, and power panels. Typed Display/Audio/Network/Bluetooth/Power writers remain Phase 5. Accessibility and Input have no hostable panel. The overlay plugin launches this window instead of dismissing into floating panels.

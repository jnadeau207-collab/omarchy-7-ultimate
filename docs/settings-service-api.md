...
- `org.omarchy.Settings` hosts the existing Network, Bluetooth, and Power panels plus the image picker on Personalization. Those hosted surfaces still run `Process` / `execDetached` / `hyprctl` / `bash -c`. Display and Sound read the existing Fabric `display.inspect` / `audio.inspect` inventories (connector/mode/scale; sinks/mute/channel volume) instead of embedding the Process monitor and audio panels. Typed Display/Audio/Network writers remain Phase 5. Accessibility and Input have no hostable panel. The overlay plugin launches this window instead of dismissing into floating panels.

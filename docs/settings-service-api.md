...
- `org.omarchy.Settings` hosts the existing Power panel plus the image picker on Personalization. Those hosted surfaces still run `Process` / `execDetached` / `hyprctl` / `bash -c`. Display, Sound, Network, and Bluetooth read the existing Fabric `display.inspect` / `audio.inspect` / `network.inspect` / `bluetooth.inspect` inventories (connector/mode/scale; sinks/mute/channel volume; Wi-Fi radio/interfaces; controller/devices) instead of embedding the Process monitor, audio, nmcli, and bluetoothctl panels. Typed Display/Audio/Network/Bluetooth writers remain Phase 5. Accessibility and Input have no hostable panel. The overlay plugin launches this window instead of dismissing into floating panels.

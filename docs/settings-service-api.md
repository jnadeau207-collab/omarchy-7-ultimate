...
- `org.omarchy.Settings` hosts the existing Display, Sound, Network, Bluetooth, and Power panels inside Settings chrome. Those panels still run `Process` / `execDetached` / `hyprctl` / `bash -c`. They are not yet typed `shell/services/` domains. The overlay plugin launches this window instead of dismissing into floating panels.

...
- `org.omarchy.Settings` hosts the existing Bluetooth and Power panels plus the image picker on Personalization. Those hosted surfaces still run `Process` / `execDetached` / `hyprctl` / `bash -c`. Display, Sound, and Network read the existing Fabric `display.inspect` / `audio.inspect` / `network.inspect` inventories (connector/mode/scale; sinks/mute/channel volume; Wi-Fi radio/interfaces) instead of embedding the Process monitor, audio, and nmcli panels. Typed Display/Audio/Network writers remain Phase 5. Accessibility and Input have no hostable panel. The overlay plugin launches this window instead of dismissing into floating panels.

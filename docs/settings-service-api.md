...
- `org.omarchy.Settings` hosts the existing Sound, Network, Bluetooth, and Power panels plus the image picker on Personalization. Those hosted surfaces still run `Process` / `execDetached` / `hyprctl` / `bash -c`. Display reads the existing Fabric `display.inspect` inventory (connector, mode, scale, position) instead of embedding the Process/hyprctl monitor panel. Typed Display/Audio/Network writers remain Phase 5. Accessibility and Input have no hostable panel. The overlay plugin launches this window instead of dismissing into floating panels.

...
Until the remaining seams are complete, Fabric `packages.install` / `packages.remove` stay hermetic. They are not LIVE. Software Center catalog cards can install or remove curated and signed-repo identities through this session's `omarchy-fabric-session-apply` `software-install` / `software-remove` actions, which call `omarchy-pkg-add` / `omarchy-pkg-drop`. Elevated auth stays on that session helper. It never enters Fabric durable evidence. Flatpak, AUR, AppImage, and web-app rows stay unavailable. Software Center is not present as product. Recipe execution and VM provisioning stay not live.

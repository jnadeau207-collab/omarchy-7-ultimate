...
It also exposes complete preflight, execute, validate, and undo hooks for `directory.create`, `entry.rename`, `entry.trash`, `trash.restore`, `mount.connect`, and `mount.disconnect`, plus a launch-plane `entry.open` hook (identity-scoped `xdg-open`; listing unchanged; no undo). The central registry names the runtime hooks `apply` and `rollback`; the provider keeps those as exact aliases of execute and undo so the lifecycle vocabulary remains explicit to future durable-operation consumers.

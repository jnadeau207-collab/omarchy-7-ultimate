...
It also exposes complete preflight, execute, validate, and undo hooks for `directory.create`, `entry.rename`, `entry.copy`, `entry.trash`, `trash.restore`, `mount.connect`, and `mount.disconnect`, plus a launch-plane `entry.open` hook (identity-scoped `xdg-open`; listing unchanged; no undo). The central registry names the runtime hooks `apply` and `rollback`; the provider keeps those as exact aliases of execute and undo so the lifecycle vocabulary remains explicit to future durable-operation consumers.
...
`files.trash.restore` is write-plane reachable (`files-trash-restore` helper). Real preflight reads the trashed entry's `.trashinfo` and scopes the original destination directory, the listing-diff inverse of `entry.trash`. Risk stays `consequential`. SHELL cannot hold a standing grant. Catalog claim is partial; humanRoute is planned empty. Inspect inventory still stores `entry.trash` as null. Files does not offer a Restore control. The `files.entry.delete` write plane exists but is not shell-authorizable (`deleteAuthorized=false`). Recycle / Empty Bin / `files.trash.manage` residual stays OPEN. Recycle Bin is not product-complete.

...
Trash is recoverable, not permanent deletion. `entry.trash` preserves the original location, parent identity, and relative path in typed recovery metadata. `trash.restore` refuses to overwrite an occupied destination or restore beneath a missing or unsafe parent. No permanent-delete capability exists in this tranche.

...
`files.entry.copy` is write-plane reachable (`files-entry-copy` helper). Real preflight binds source identity inside the destination directory listing and writes a scoped regular-file replica into a writable Files location. The helper refuses collision, identity drift, traversal, symlink targets, directories, and Trash. Risk is `low`. SHELL can hold a standing grant. Catalog claim is partial; humanRoute is visible `Files > Copy and Paste`. Agent stays unavailable. Files Copy and Paste use that plane with in-app staging. It does not invent LIVE Cut, folder copy, thumbnails, or an OS clipboard.

`files.entry.move` is write-plane reachable (`files-entry-move` helper). Real preflight binds source identity inside the destination directory listing and relocates a scoped regular file into a writable Files location. The helper refuses collision, identity drift, traversal, symlink targets, directories, missing parents, and Trash. Risk is `consequential`. SHELL cannot hold a standing grant (`grant.shell-consequential`). Catalog claim is partial; humanRoute is planned empty. Agent stays unavailable. Files keeps `cutAuthorized=false` and does not show LIVE Cut under the shell principal. It does not invent an OS clipboard or folder copy.

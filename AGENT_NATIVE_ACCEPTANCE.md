...
**Historical freeze:** this matrix was audited at `1334ba30` on 2026-08-26. Several rows are now false against the tree (the capability catalog exists; Agent Center exists as a read-only host). Do not treat the table below as live status. After W0, remaining fabric work is Phase 2 of `plans/project-ultimate.md`. Phase 2 leftover (rows 1, 8, 9, 11) is proved by `omarchy-dev-capability-check`, `test/shell.d/capability-catalog-test.sh`, and the fabric managed-work / sandbox suites, not by editing this table until it looks green. The OS-level pass bar in **Pass bar** still applies.
